...
/** Values compose.prod.yml interpolates that no Cloud process reads. */
const composeInputs: Record<string, { value: string; doc: string }> = {
...
const developmentEnv = (registries: Registry[]): string => {
  ...
  const compose = envLine("CLOUD_MAIL_APP_CREDENTIAL", { schema: envString, doc: composeInputs.CLOUD_MAIL_APP_CREDENTIAL!.doc }, "", true);
  return `${[
...
const outputs = async (): Promise<Record<string, string>> => {
  ...
  return {
    ".env.example": developmentEnv(registries),
