...
const validateEvals = (violations: Finding[], file: string, skillName: string, requireExpectations = false): void => {
  if (!existsSync(file)) {
...
export const rule: Rule = {
  name: "skills",
  description: "Repository skills and the published cloud-dev Fibel skill stay valid",
  run: async ({ workspaceRoot }) => {
    ...
    if (!isDirectory(skillsRoot)) return [{ file: skillsRoot, message: "Missing skills directory at cloud/skills." }];
    ...
    validateEvals(violations, join(publishedEvalsRoot, "cloud-dev", "evals.json"), "cloud-dev", true);
