...
const catalog = i18n.define({
  ...
  messages: {
    en: {
      ...
      noBookWithRef: ({ ref }: { ref: string }) => `No contact book has the ID or exact name "${ref}".`,
      noContactWithName: ({ name, book }: { name: string; book: string }) => `No contact in ${book} has the ID or exact name "${name}".`,
      noContactWithId: ({ ref }: { ref: string }) => `No contact has the ID "${ref}". Use a contact ID or <book>:<name>.`,
      noContactWithEmail: ({ email }: { email: string }) => `No readable contact has the email address "${email}".`,
    ...
    de: {
      ...
      noBookWithRef: ({ ref }) => `Kein Kontaktbuch hat die ID oder den exakten Namen „${ref}“.`,
      noContactWithName: ({ name, book }) => `Kein Kontakt in ${book} hat die ID oder den exakten Namen „${name}“.`,
      noContactWithId: ({ ref }) => `Kein Kontakt hat die ID „${ref}“. Verwende eine Kontakt-ID oder <buch>:<name>.`,
      noContactWithEmail: ({ email }) => `Kein lesbarer Kontakt hat die E-Mail-Adresse „${email}“.`,
