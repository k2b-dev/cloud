...
export default function AiUsageCharts(props: { timeline: AiUsageReport["timeline"]; range: AiUsageReport["overview"]["range"] }) {
  ...
  return (
    <div class="grid min-w-0 gap-2">
      ...
      <DataPanel
        ...
        isEmpty={!props.timeline.some((point) => point.tokens > 0)}
        empty={t().noTokens}
      >
        <Chart
          kind="line"
          ...
          series={[{ label: t().tokens, data: props.timeline.map((point) => ({ x: new Date(point.bucket).getTime(), y: point.tokens })) }]}
          xAxis={xAxis}
