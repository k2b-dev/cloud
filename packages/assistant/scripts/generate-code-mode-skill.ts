...
const instructions = document.instructions.replace(/\]\(references\/([^\s)]+)\)/g, `](/skills/${document.name}/references/$1)`);
const template = { key: "assistant:code-mode", version: 52, ...document, instructions, references };
...
import type { AiSkillTemplate } from "./skills";
export const ASSISTANT_DATA_ANALYSIS_SKILL = ${JSON.stringify({ key: "assistant:data-analysis", version: 6, ...analysis }, null, 2)} satisfies AiSkillTemplate;
`;
const analysisDestination = new URL("../../cloud/src/ai/data-analysis-skill.ts", import.meta.url);
if (process.argv.includes("--check")) {
  if (!(await Bun.file(analysisDestination).exists()) || (await Bun.file(analysisDestination).text()) !== analysisOutput)
    throw new Error("Data analysis skill snapshot is stale");
