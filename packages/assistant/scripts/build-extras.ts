...
const hostProcess = await Bun.build({entrypoints:[resolve(import.meta.dir,"../src/cli/code-host-process.ts")],target:"bun",format:"esm",external:["playwright"]});
...
for (const name of ["playwright","playwright-core"]) {
  const packagePath = Bun.resolveSync(`${name}/package.json`,dirname(Bun.resolveSync("playwright/package.json",resolve(import.meta.dir,".."))));
