...
import { type ImportDatabase, type ImportRow, importRows } from "./import-rows";
...
test("retries an interrupted multi-batch import without duplicates and rejects changed input", async () => {
  ...
  await expect(importRows(db, rows, async () => {})).rejects.toThrow("connection lost");
  expect(stored.size).toBe(200);
  fail = false;
  expect(await importRows(db, rows, async () => {})).toEqual({ inserted: 2300, skipped: 200 });
