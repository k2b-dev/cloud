...
export function reconcile(bookings: Booking[], invoices: Invoice[]) {
  ...
  for (const invoice of unique) {
    const group = byReference.get(invoice.reference) ?? [];
    group.push(invoice);
    byReference.set(invoice.reference, group);
  }
  const potentialOwners = new Map<Invoice, Set<Booking>>();
  for (const booking of bookings) {
    const groups = [...new Set(booking.references)].map((ref) =>
      (byReference.get(ref) ?? []).filter((invoice) => invoice.currency === booking.currency),
    );
    ...
    for (const invoice of selected) {
      const owners = potentialOwners.get(invoice) ?? new Set<Booking>();
      owners.add(booking);
      potentialOwners.set(invoice, owners);
    }
  }
  const used = new Set<Invoice>(),
    seenBookings = new Set<string>();
  const matches: Match[] = [];
  for (const booking of bookings) {
    ...
    matches.push({
      booking,
      invoices: refs.length
        ? selected
        : unique.filter((invoice) => !used.has(invoice) && invoice.currency === booking.currency && invoice.cents === booking.cents),
      difference,
      status,
      reason,
    });
...
export function exportRows(result: ReturnType<typeof reconcile>) {
  ...
  const confirmed = result.matches
    .filter((match) => match.status === "confirmed")
    .flatMap((match) =>
      match.invoices.map((invoice, index) => ({
        bank_id: match.booking.id,
        bank_file: match.booking.source.file,
        bank_page: match.booking.source.page,
        // The aggregate payment appears only once, never once per invoice.
        bank_amount: index === 0 ? euro(match.booking.cents) : "",
        currency: invoice.currency,
        invoice: invoice.reference,
        invoice_amount: euro(invoice.cents),
        file: invoice.source.file,
        page: invoice.source.page,
        reason: match.reason,
      })),
    );
  const open = result.matches
    .filter((match) => match.status !== "confirmed")
    .map((match) => ({
      reference: match.booking.id,
      file: match.booking.source.file,
      page: match.booking.source.page,
      amount: euro(match.booking.cents),
      reason: match.reason,
    }));
  for (const invoice of result.unmatched)
    open.push({
      reference: invoice.reference,
      file: invoice.source.file,
      page: invoice.source.page,
      amount: euro(invoice.cents),
      reason: "Unassigned invoice",
    });
  for (const group of result.conflicts)
    for (const invoice of group)
      open.push({
        ...
        page: invoice.source.page,
        amount: euro(invoice.cents),
        reason: "Conflicting duplicate invoice",
      });
  for (const group of result.duplicates)
    for (const invoice of group.slice(1))
      open.push({
        reference: invoice.reference,
        file: invoice.source.file,
        page: invoice.source.page,
        amount: euro(invoice.cents),
        reason: "Duplicate document; excluded from totals",
      });
