...
const app: ArtifactSummary = { id: "App001", kind: "app", title: "Banking", description: "Rechnungen", revision: 1, publishedRevision: 1, permission: "read", updatedAt: "", forkedFromId: null, forkedFromRevision: null };
...
test("Studio search delegates identity, query and bound to the authorized catalog and links to app details", async () => {
  ...
  expect(result.data.data[0]).toMatchObject({ ref: { type: "assistant.app", id: "App001" }, preview: "Rechnungen", links: [{ rel: "open", href: "/app/assistant/apps/App001" }] });
});

test("Studio search rejects unsupported scopes and service identities before catalog access", async () => {
  const store = { list: async () => { throw new Error("must not read"); } };
  ...
  expect((await searchAssistantApps(input, { ...context, accessSubject: { type: "service_account", serviceAccountId: "svc" } }, store)).ok).toBe(false);
});
...
test("Studio search button uses the registered app search tag", () => {
  ...
  expect(assistantAppsSearchOptions("de")).toEqual({ query: "", scope: { appId: "assistant", tag, label: "Studio", icon: "ti ti-app-window" } });
});

test("all-chat search stays restricted to chats when projects and Studio also provide search", () => {
  expect(assistantSearchOptions("en")).toEqual({ query: "", scope: { appId: "assistant", tag: "chat", label: "Chats", icon: "ti ti-messages" } });
  expect(assistantSearchOptions("en", { id: "Chat01", title: "Chat" })).toMatchObject({ query: "", scope: { ref: { type: "assistant.chat", id: "Chat01" } } });
});
