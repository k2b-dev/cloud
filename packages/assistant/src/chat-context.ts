...
export const loadAssistantChatContextSnapshot = async (
  ...
  while (sourcePage.nextCursor) {
    const next = await aiConversations.listConversationSources({
      conversationId: conversation.id,
      limit: 100,
      before: sourcePage.nextCursor,
    });
        ...
        ? check.status === "ready"
          ...
          : check.status === "error"
            ...
            : de
              ...
              : "Run incomplete"
        : de
          ? "Revision noch nicht ausgeführt"
          : "Revision not run yet";
    const preview = [
      app.kind === "app" ? "App" : de ? "Skript" : "Script",
      `R${app.revision}`,
      app.publishedVersion ? `${de ? "Veröffentlicht" : "Published"} v${app.publishedVersion}` : de ? "Entwurf" : "Draft",
      status,
      app.description,
    ]
      .filter(Boolean)
