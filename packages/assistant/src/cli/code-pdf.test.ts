...
test("CLI PDF uses binary multipart, cancels an individual request and continues", async () => {
  ...
  const host = await createCliCodeHost({ fetch: async (input, init) => {
    ...
    if (url.pathname.endsWith("/runtime/pdf")) {
      ...
      if (request.operation === "render") {
        ...
        return new Promise<Response>((_resolve, reject) => signal.addEventListener("abort", () => { aborted = true; reject(new DOMException("Cancelled", "AbortError")); }, { once: true }));
      }
  ...
  try {
    const result = await host.execute({ name: "code_run", callId: "pdf", conversationId: "aBc234", turnId: "aBc234", args: { code: `export default async()=>{
    ...
    expect(result).toMatchObject({ status: "ready", output: JSON.stringify({ cancelled: true, type: "application/pdf", content: "%PDF-output" }) });
