...
export function openPublicationInfo(input: {
  id: string;
  revision: number;
  published: boolean;
  version?: number | null;
    ...
    (close) => {
      ...
      return (
        <div class="flex flex-col gap-4">
          ...
          <Show when={!input.published && input.unsavedChanges}>
            <NoticeCard tone="warning" title={advancedMessages.resolve([input.locale]).t.saveBeforePublish} />
          </Show>
          <Show when={error()}>
            <NoticeCard tone="danger" title={error()} />
          </Show>
          <Show when={!input.published}>
            <div class="flex justify-end">
              <Button
                loading={busy()}
                disabled={busy() || input.unsavedChanges}
                onClick={async () => {
