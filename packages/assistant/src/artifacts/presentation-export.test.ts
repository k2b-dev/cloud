...
test("document export retains filter values and all rows without executable controls", () => {
  const nodes = parse([
    ...
    { id: "table", type: "table", rowKey: "id", columns: [{ key: "value", label: "Value" }], rows: Array.from({ length: 70 }, (_, i) => ({ id: String(i), value: i })) },
  ]);
  const html = presentationHtml(nodes, "Report", "de");
  expect(html).toContain("Quantity"); expect(html).toContain("<p>7</p>");
...
test("charts export SVG and keep actual source context", () => {
  const [node] = parse([{ id: "chart", type: "explorer", columns: [{ key: "value", label: "Value" }], data: {
    rowKey: "id", rows: [{ id: "a", label: "A", value: 12 }], chart: { kind: "bar", category: "label", value: "value" },
    context: { mode: "snapshot", asOf: "2026-09-20T10:00:00Z", sources: [{ label: "Reviewed CSV" }], status: "partial", note: "Missing one day" },
  } }]);
  expect(presentationChartSvg(node!, "en")).toContain("<svg");
  const html = presentationHtml([node!], "Report", "en");
  expect(html).toContain("Reviewed CSV"); expect(html).toContain("Missing one day");
...
test("rejects cyclic presentations and unsettled export instead of losing content", () => {
  ...
  expect(() => ChatPresentationInput.parse({ conversationId: "abc234", callId: "x", title: "x", code: "x", nodes, inputs: [] })).toThrow("Cyclic");
