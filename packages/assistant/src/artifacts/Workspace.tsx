import type { AiChatTaskView, AiProject } from "@k2b/cloud/ai";
import { conversationFileSource } from "@k2b/cloud/ai/solid";
import { Button, Dropdown, FileTree, FileView, NoticeCard, Placeholder, prompts, Tabs, useLocale } from "@k2b/ui";
...
export function createArtifactWorkspace(initialHref?: string) {
  ...
  const syncUrl = () =>
    window.history.replaceState(
      ...
      "",
      workspaceSelectionHref(window.location.href, state().tabs.find((tab) => tab.key === state().active) ?? null),
    );
  const open = (tab: WorkspaceTab) => {
    setState((state) => openWorkspaceTab(state, tab));
    setMobile("workspace");
    syncUrl();
  };
  return {
    ...
    rename: (key: string, title: string) =>
      setState((state) => ({ ...state, tabs: state.tabs.map((tab) => (tab.key === key ? { ...tab, title } : tab)) })),
    restore: () => {
      const tab = workspaceSelectionFromHref(window.location.href);
      if (tab) open(tab);
    },
    select: (key: string) => {
      setState((state) => ({ ...state, active: key }));
      syncUrl();
    },
...
function SourceFile(props: { tab: Extract<WorkspaceTab, { kind: "source" }>; dirty: (value: boolean) => void }) {
  ...
  return (
    <>
      <Show when={conflict()}>
        <NoticeCard tone="warning" title={t().CONFLICT} detail={t().sourceConflict} />
        <Show when={loadError()}>
          <NoticeCard tone="danger" title={t().loadFailed} detail={loadError()} />
        </Show>
        <div class="flex gap-2">
          <Button
            ...
            onClick={async () => {
              ...
              setLoadError("");
              try {
                mutate(await artifactClient.get(props.tab.artifactId));
                setRefresh((value) => value + 1);
                setConflict(false);
              } catch (error) {
                setLoadError(error instanceof Error ? error.message : t().REQUEST_FAILED);
              } finally {
                setLoading(false);
              }
            }}
                ...
                : async (content) => {
                    ...
                    try {
                      const updated = await artifactClient.update(current.id, {
                        ...
                        source: {
                          ...current.source,
                          files: current.source.files.map((file) => (file.path === props.tab.path ? { ...file, content } : file)),
                        },
                      });
                      mutate(updated);
                      setConflict(false);
...
function ChatFile(props: { tab: Extract<WorkspaceTab, { kind: "file" }>; refreshKey: string; dirty: (value: boolean) => void }) {
  ...
  createEffect(() => {
    if (!dirty()) setRevision(props.refreshKey);
  });
  return (
    <FileView
      file={{ path: props.tab.path }}
...
function SourceDirectory(props: { artifactId: string; open: (path: string) => void }) {
  ...
  return (
    <Show
      ...
      fallback={
        <Placeholder
          state={bundle.error ? "error" : "loading"}
          description={bundle.error?.message}
          action={bundle.error ? <Button onClick={() => void refetch()}>{t().refresh}</Button> : undefined}
        />
      }
    >
      {(data) => (
        <FileTree
          label={t().source}
          entries={data().source.files.map((file) => ({ path: file.path }))}
          onSelect={(file) => {
            if (file.kind !== "folder") props.open(file.path);
          }}
        />
      )}
    </Show>
...
export function ArtifactWorkspace(props: {
  ...
  userId: string;
  refreshKey: string;
  onEditTask: (task: AiChatTaskView, repair: boolean) => void;
  menuItems?: import("@k2b/ui").DropdownItem[];
  ...
  async function close(key = state().active) {
    if (!key) return;
    if (props.controller.isDirty(key) && !(await prompts.confirm(t().unsavedMessage, { title: t().unsavedTitle, variant: "danger" })))
      return;
    ...
    <div class="artifact-workspace">
      <div class="artifact-workspace__tabs">
        <Tabs
          variant="pill"
          trailing={
            <Show when={props.menuItems?.length}>
              <Dropdown.Root items={props.menuItems ?? []}>
                <Dropdown.Trigger iconOnly label={t().open}>
          ...
          value={() => state().active ?? ""}
          onValueChange={props.controller.select}
          ariaLabel={t().workspace}
          options={state().tabs.map((tab) => ({
            value: tab.key,
            label: title(tab),
            icon: tab.kind === "task" ? "ti ti-calendar-time" : tab.kind === "app" ? "ti ti-app-window" : "ti ti-file-code",
            onClose: () => void close(tab.key),
            closeLabel: `${t().close}: ${title(tab)}`,
          }))}
        />
        <Button class="artifact-mobile-toggle" size="sm" variant="ghost" onClick={() => props.controller.setMobile("chat")}>
          {t().chat}
        </Button>
      </div>
      <For each={state().tabs.map((tab) => tab.key)}>
        {(key) => {
          ...
          return (
            <div class="artifact-workspace__tab" role="tabpanel" hidden={state().active !== key} aria-label={title(tab)}>
              <ErrorBoundary
              ...
              >
                {tab.kind === "task" ? (
                  <AssistantTaskDetail
                    ...
                    onTitle={(title) => props.controller.rename(key, title)}
                    onOpenRun={(taskId, occurrenceId) => void openAssistantTaskRun(taskId, occurrenceId, live)}
                  />
                ) : tab.kind === "context" ? (
                  <AssistantChatContextContent
                    chatId={tab.conversationId}
                    project={
                      tab.project !== undefined ? tab.project : tab.conversationId === props.conversationId ? props.project : undefined
                    }
                  ...
                  <ArtifactPanel
                    ...
                    browseSource={() =>
                      props.controller.open({
                        kind: "view",
                        key: `${tab.artifactId}:source`,
                        title: t().source,
                        render: () => (
                          <SourceDirectory
                            artifactId={tab.artifactId}
                            open={(path) => props.controller.open(sourceTab(tab.artifactId, path))}
                          />
                        ),
                      })
                    }
                  ...
                  <ChatFile tab={tab} refreshKey={props.refreshKey} dirty={(value) => props.controller.setDirty(key, value)} />
                ) : (
