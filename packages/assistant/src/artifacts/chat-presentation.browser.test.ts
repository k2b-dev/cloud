...
test("chat preview survives reload without execution; controls run on demand and exports are static", async () => {
  const build = Bun.spawn(["bun", new URL("./workspace-browser-build.ts", import.meta.url).pathname, "./chat-presentation-browser-harness.tsx"], {stdout:"pipe",stderr:"pipe"});
  ...
  const server = Bun.serve({hostname:"127.0.0.1",port:0,async fetch(req) {
    ...
    if(path==="/api/assistant/artifacts/runtime/compile"){starts++;return Response.json(source);}
