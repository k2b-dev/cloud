...
test("chat preview survives reload without execution; controls run on demand and exports are static", async () => {
  ...
  try {
    ...
    await page.locator(".assistant-chat-presentation__body").evaluate(el => { el.scrollTop = 80; });
    expect((await header.boundingBox())!.y).toBeLessThan(headerTop);
    await page.locator(".assistant-chat-presentation__body").evaluate(el => { el.scrollTop = 0; });
