...
test("Code AI uses the caller's model access and allowance and propagates cancellation", async () => {
  ...
  try {
    expect(await runCodeAi({ kind: "generate_text", prompt: "Summarize", input: "Text" }, { resourceId: "aBc234" }, identity, abort.signal)).toBe("Summary");
    expect(select).toHaveBeenCalledWith(identity.accessSubject, undefined);
    expect(execute.mock.calls[0]?.[1]).toMatchObject({ usageSubject: identity.accessSubject, attribution: { userId: user(identity).id }, signal: abort.signal, requestedModelId: "allowed-model", taskPrefix: "code" });
    select.mockRejectedValueOnce(new Error("Model not allowed"));
    await expect(runCodeAi({ kind: "generate_text", prompt: "Test" }, { resourceId: "aBc234" }, identity, abort.signal)).rejects.toThrow("Model not allowed");
    ...
    await expect(runCodeAi({ kind: "generate_text", prompt: "Test" }, { resourceId: "aBc234" }, identity, new AbortController().signal)).rejects.toMatchObject({ code: "ACCESS_DENIED" });
    expect(execute).toHaveBeenCalledTimes(1);
  } finally { authorize.mockRestore(); select.mockRestore(); execute.mockRestore(); }
});
