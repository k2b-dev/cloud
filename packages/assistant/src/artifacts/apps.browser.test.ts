...
test("Studio lists open details with all management actions; publication and versions remain usable", async () => {
  ...
  const item = {
    ...
    description: "Calculate tips precisely.",
    revision: 2,
    permission: "admin",
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (path.endsWith("/publish")) {
        publicationNotes.push((await request.json()).note);
        mutations.push("publish");
        publishedRevision = 2;
        return Response.json({ publishedRevision });
      }
      if (path.endsWith("/unpublish")) {
        publishedRevision = null;
        return Response.json({ publishedRevision });
      }
      if (path.endsWith("/edit-chat")) {
        mutations.push("edit-chat");
        return Response.json({ href: "/edited" });
      }
      ...
      if (path.endsWith("/versions"))
        return Response.json({
          items: emptyHistory
            ? []
            : [{ version: 1, revision: 1, note: "Initial calculator", createdAt: new Date().toISOString(), authorId: "test" }],
          page: 1,
          hasNext: false,
        });
      ...
      if (path === "/api/assistant/artifacts") {
        ...
        return Response.json({
          items: removed
            ? []
            : longCatalog
              ? Array.from({ length: 30 }, (_, i) => ({ ...item, id: `App${i}`, title: `Catalog app ${i}`, publishedRevision }))
              : [{ ...item, publishedRevision }],
          page: 1,
          hasNext: false,
        });
      }
      if (path.startsWith("/api/assistant/artifacts/") && request.method === "DELETE") {
        removed = true;
  ...
  try {
    ...
    await page
      .getByRole("dialog", { name: "Studio", exact: true })
      .getByText("No apps yet. Ask Assistant to build one.", { exact: true })
      .waitFor();
