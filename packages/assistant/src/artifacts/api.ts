...
const revision = (c: Context<AuthContext>) =>
  c.req.query("revision") === undefined ? undefined : z.coerce.number().int().positive().parse(c.req.query("revision"));
...
const RevisionQuery = z.object({
  ...
  published: z.enum(["true"]).optional(),
});
const Grant = z
  .object({
  ...
  .strict()
  .refine((value) => value.principal.type !== "public" || value.permission === "read", "Public access only supports read");

export const createArtifactServiceRoutes = (caller: (context: Context<AuthContext>) => CapabilityCaller = capabilityCaller) =>
  new Hono<AuthContext>()
    .use("*", (c, next) =>
      c.req.path.endsWith("/storage/file") ||
      c.req.path.endsWith("/runtime/pdf") ||
      /\/runtime\/capabilities\/[^/]+\/stream\/(read|write|status|abort)$/.test(c.req.path)
        ? next()
        : bodyLimit({ maxSize: c.req.path.includes("/storage") ? STORAGE_TRANSPORT_BYTES : LIMITS.rpcBytes })(c, next),
    )
    .use("*", async (c, next) => {
      ...
      if (error instanceof GotenbergRenderError) {
        const code =
          error.code === "not_configured"
            ...
            : error.code === "timeout"
              ? "PDF_TIMEOUT"
              : error.code === "html_too_large" || error.code === "pdf_too_large"
                ? "PDF_LIMIT"
        ...
        return respond(c, {
          ok: false,
          code,
          status: code === "PDF_LIMIT" ? 413 : code === "INVALID_INPUT" ? 400 : 503,
      ...
      if (error instanceof DatabaseError) {
        ...
        const message =
          error.code === "DB_SQL_UNSUPPORTED"
            ? t.DB_SQL_UNSUPPORTED
            : error.code === "DB_SQL_PARAMS"
              ? t.DB_SQL_PARAMS
              : error.code === "DB_LIMIT"
                ? t.DB_LIMIT
                : error.code === "CONFLICT"
      ...
      const status =
        code === "TOO_MANY_REQUESTS"
          ...
          : code === "NOT_FOUND"
            ...
            : code === "ACCESS_DENIED"
              ? 403
              : code === "CONFLICT" || code === "LAST_MANAGER"
                ? 409
                : code === "REQUEST_FAILED"
                  ? 500
                  : 400;
    ...
    .get("/presentations/:presentationId", async (c) =>
      c.json(
        await chatPresentations.read(
    ...
    .post(
      ...
      bodyLimit({ maxSize: STORAGE_TRANSPORT_BYTES }),
      async (c) => {
        let input: ReturnType<typeof decodePdfRequest>;
        try {
          input = decodePdfRequest(await c.req.raw.formData());
        } catch {
    ...
    .post("/runtime/secrets/list", v("json", HttpScope), async (c) =>
      respond(c, ok(await httpService.list(c.req.valid("json"), identity(c)))),
    )
    .put("/runtime/secrets", v("json", z.object({ scope: HttpScope, secret: SecretSave }).strict()), async (c) => {
      const input = c.req.valid("json");
      return respond(c, ok(await httpService.save(input.scope, input.secret, identity(c))));
    })
    .post(
      "/runtime/secrets/remove",
      v("json", z.object({ scope: HttpScope, name: z.string(), revision: z.uuid() }).strict()),
      async (c) => {
        const input = c.req.valid("json");
        return respond(c, ok(await httpService.remove(input.scope, input.name, input.revision, identity(c))));
      },
    )
    ...
    .post("/runtime/http/:callId", v("json", z.object({ approved: z.boolean() }).strict()), async (c) =>
      respond(
        c,
        ok(await httpService.execute(z.uuid().parse(c.req.param("callId")), c.req.valid("json").approved, identity(c), c.req.raw.signal)),
      ),
    )
    .get("/project-links/:projectId", v("query", PageQuery.extend({ available: z.enum(["true", "false"]).optional() })), async (c) =>
      respond(
        c,
        ok(
          await artifacts.projectApps(
            z.string().min(1).max(80).parse(c.req.param("projectId")),
            identity(c),
            page(c),
            c.req.valid("query").q,
            c.req.valid("query").available === "true",
          ),
        ),
      ),
    )
    .get("/", v("query", PageQuery), async (c) => respond(c, ok(await artifacts.list(identity(c), page(c), c.req.valid("query").q))))
    .get("/admin/resources", v("query", PageQuery.extend({ search: z.string().max(120).optional() })), async (c) =>
      respond(c, ok(await artifactAdmin.list(identity(c), page(c), c.req.valid("query").search))),
    )
    ...
    .post("/admin/resources/:id/access", v("json", Grant), async (c) =>
      respond(
        c,
        ok(await artifacts.grant(id(c), c.req.valid("json").principal, c.req.valid("json").permission, adminIdentity(identity(c)))),
      ),
    )
    .put("/admin/resources/:id/access/:accessId", v("json", z.object({ permission: Level.nullable() }).strict()), async (c) =>
      respond(
        c,
        ok(
          await artifacts.changeGrant(
            id(c),
            z.uuid().parse(c.req.param("accessId")),
            c.req.valid("json").permission,
            adminIdentity(identity(c)),
          ),
        ),
      ),
    )
    .get("/admin/storage/settings", async (c) => respond(c, ok(await storageSettings.read(identity(c)))))
    .put("/admin/storage/settings", v("json", StorageSettings), async (c) =>
      respond(c, ok(await storageSettings.write(c.req.valid("json"), identity(c)))),
    )
    .get("/admin/database/settings", async (c) => respond(c, ok(await artifactDatabase.settings(identity(c)))))
    .put("/admin/database/settings", v("json", DatabaseSettings), async (c) =>
      respond(c, ok(await artifactDatabase.configure(c.req.valid("json"), identity(c)))),
    )
    .post("/admin/database/test", v("json", DatabaseSettings), async (c) =>
      respond(c, ok(await artifactDatabase.configure(c.req.valid("json"), identity(c), true))),
    )
    .post("/:id/database/maintenance/connect", async (c) =>
      respond(c, ok(await artifactDatabase.connect(id(c), identity(c), c.req.raw.signal, true))),
    )
    .post("/:id/database/maintenance", v("json", DatabaseRequest), async (c) =>
      respond(c, ok(await artifactDatabase.call(id(c), c.req.valid("json"), identity(c), c.req.raw.signal, "maintenance"))),
    )
    ...
    .post("/files/stat", v("json", CODE_SOURCE_TOOLS.code_file_stat.input), async (c) => {
      ...
      return respond(
        c,
        ok(file ? { exists: true, reference: file.reference, size: file.bytes.byteLength, mediaType: file.mediaType } : { exists: false }),
      );
    })
    ...
    .post(
      ...
      v(
        "json",
        z
          .object({ confirmed: z.literal(true), expectedGeneration: z.string().regex(/^[a-f0-9]{64}$/), expectedDataRevision: z.uuid() })
          .strict(),
      ),
      async (c) => {
        const input = c.req.valid("json");
        return respond(
          c,
          ok(await artifactDatabase.clear(id(c), input.expectedGeneration, input.expectedDataRevision, identity(c), c.req.raw.signal)),
        );
      },
    )
    .post(
      "/:id/database/reset",
      v("json", z.object({ confirmed: z.literal(true), expectedGeneration: z.string().length(64).nullable() }).strict()),
      async (c) => respond(c, ok(await artifactDatabase.reset(id(c), c.req.valid("json").expectedGeneration, identity(c)))),
    )
    .get("/:id/database/export", async (c) => {
      ...
      return new Response(response.body, {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="studio-${id(c)}.sqlite"`,
          "Cache-Control": "private, no-store",
        },
      });
    })
    ...
    .post("/:id/database/inspect", v("json", DatabaseRequest), async (c) =>
      respond(c, ok(await artifactDatabase.call(id(c), c.req.valid("json"), identity(c), c.req.raw.signal, "inspect"))),
    )
    .post("/runtime/capabilities/:callId/stream/:verb", async (c) =>
      runtimeCapabilities.stream(
        c.req.param("callId"),
        z.enum(["read", "write", "status", "abort"]).parse(c.req.param("verb")),
        c.req.raw.body,
        identity(c),
        caller(c),
      ),
    )
    .post("/runtime/capabilities", v("json", RuntimeCapabilityRequest), async (c) =>
      respond(c, ok(await runtimeCapabilities.prepare(c.req.valid("json"), identity(c), caller(c)))),
    )
    .post(
      "/runtime/capabilities/:callId/resolve",
      v("json", z.object({ approved: z.boolean(), remember: z.literal("always").optional() }).strict()),
      async (c) =>
        respond(
          c,
    ...
    .post("/runtime/complete", v("json", ClientCallResult), async (c) =>
      respond(c, ok(await clientCalls.complete(c.req.valid("json"), identity(c)))),
    )
    ...
    .post(
      "/runtime/action",
      v("query", z.object({ conversationId: z.string().max(80).optional() })),
      v("json", CodeActionInput),
      async (c) => {
        const input = c.req.valid("json");
        ...
        if (draft ? bundle.sourceRevision !== input.revision : bundle.publishedVersion !== input.publishedVersion)
    ...
    .post(
      "/runtime/rename",
      v("json", z.object({ source: ArtifactSource, from: z.string().max(180), to: z.string().max(180) }).strict()),
      async (c) => {
        const input = c.req.valid("json");
        try {
          return respond(c, ok(renameSource(input.source, input.from, input.to)));
        } catch {
          return respond(c, {
            ok: false,
            status: 400,
            code: "INVALID_INPUT",
            error: artifactMessages.resolve([getLocale(c)]).t.INVALID_INPUT,
          });
        }
      },
    )
    .post("/validate", v("json", ArtifactSource), async (c) => {
      try {
        await compileArtifact(c.req.valid("json"));
        return respond(c, ok({ valid: true, diagnostics: [] }));
      } catch (error) {
        return respond(c, ok({ valid: false, diagnostics: [compilationDiagnostic(error)] }));
      }
    })
    ...
    .get("/:id", v("query", RevisionQuery), async (c) =>
      respond(
        c,
        ok(
          await artifacts.get(
            ...
            identity(c),
            revision(c),
            c.req.query("published") === "true",
            c.req.query("version") === undefined ? undefined : z.coerce.number().int().positive().parse(c.req.query("version")),
          ),
        ),
      ),
    )
    .post(
      "/:id/publish",
      v("json", z.object({ expectedRevision: z.number().int().positive(), note: PublicationNote }).strict()),
      async (c) =>
    ...
    .post(
      "/:id/restore",
      v("json", z.object({ version: z.number().int().positive(), expectedRevision: z.number().int().positive() }).strict()),
      async (c) =>
        respond(c, ok(await artifacts.restore(id(c), c.req.valid("json").version, c.req.valid("json").expectedRevision, identity(c)))),
    )
    .patch("/:id/metadata", v("json", ArtifactMetadata), async (c) =>
      respond(c, ok(await artifacts.metadata(id(c), c.req.valid("json"), identity(c)))),
    )
    ...
    .post("/:id/edit-chat", v("query", z.object({ intent: z.literal("customize").optional() })), async (c) =>
      respond(
        c,
        ok(
          await artifacts.editChat(
            id(c),
            identity(c),
            c.req.valid("query").intent === "customize" ? artifactMessages.resolve([getLocale(c)]).t.customizePrompt : undefined,
          ),
        ),
        201,
      ),
    )
    .put("/:id/files", v("json", ArtifactFile), async (c) =>
      respond(c, ok(await artifacts.writeFile(id(c), c.req.valid("json").path, c.req.valid("json").content, identity(c)))),
    )
    ...
    .get("/:id/storage/file", v("query", StorageFileQuery), async (c) => {
      ...
      const result = await artifacts.storage(
        id(c),
        { area: "files", operation: "read", key: input.key },
        identity(c),
        input.management === "true",
      );
      if (!("item" in result) || !result.item?.data) throw new ArtifactError("NOT_FOUND");
      return new Response(new Uint8Array(result.item.data), {
        headers: {
    ...
    .put("/:id/storage/file", v("query", StorageFileQuery), async (c) => {
      ...
      return bodyLimit({
        maxSize: maximum,
        onError: (c) => c.json({ code: "STORAGE_FULL", message: artifactMessages.resolve([getLocale(c)]).t.STORAGE_FULL }, 413),
      })(c, async () => {
        const data = new Uint8Array(await c.req.arrayBuffer());
        const result = await artifacts.storage(
          id(c),
          { area: "files", operation: "write", key: input.key, mediaType: c.req.header("content-type") ?? "application/octet-stream" },
          identity(c),
          input.management === "true",
          data,
        );
    ...
    .post(
      "/:id/storage",
      v("query", z.object({ conversationId: z.string().max(80).optional() })),
      v("json", StorageJsonRequest),
      async (c) => respond(c, ok(await artifacts.storage(id(c), c.req.valid("json"), identity(c)))),
    )
    .post("/:id/storage/manage", v("json", StorageJsonRequest), async (c) =>
      respond(c, ok(await artifacts.storage(id(c), c.req.valid("json"), identity(c), true))),
    )
    .post(
      "/:id/storage/clear",
    ...
    .put("/:id/projects/:projectId", v("json", z.object({ linked: z.boolean() }).strict()), async (c) =>
      respond(
        c,
        ok(
          await artifacts.linkProject(
            id(c),
            z.string().min(1).max(80).parse(c.req.param("projectId")),
            c.req.valid("json").linked,
            identity(c),
          ),
        ),
      ),
    ...
    .put("/:id/access/:accessId", v("json", z.object({ permission: Level.nullable() }).strict()), async (c) =>
      respond(
        c,
        ok(await artifacts.changeGrant(id(c), z.uuid().parse(c.req.param("accessId")), c.req.valid("json").permission, identity(c))),
      ),
    );
