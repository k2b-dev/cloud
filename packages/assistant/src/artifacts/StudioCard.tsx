import { Button, Dropdown, type DropdownItem, IconButtonLink, Paper, StatusBadge, useLocale } from "@k2b/ui";
...
export function StudioCard(props: {
  ...
  return (
    <Paper class="assistant-studio-card" style={studioColor(props.item.id)}>
      <div class="assistant-studio-card__top" classList={{ "assistant-studio-card__top--title-only": !props.item.description?.trim() }}>
        <div class="assistant-studio-card__icon" aria-hidden="true">
          <i class={props.item.icon ?? "ti ti-app-window"} />
        </div>
        <div class="assistant-studio-card__copy">
          ...
          <Show when={props.item.description?.trim()}>
            <p class="assistant-studio-card__description">{props.item.description}</p>
          </Show>
        </div>
        <div class="assistant-studio-card__menu">
          <Dropdown.Root items={props.menu}>
            <Dropdown.Trigger iconOnly variant="ghost" label={`${t().actions} · ${props.item.title}`} disabled={props.busy}>
              <i class="ti ti-dots" />
            </Dropdown.Trigger>
          </Dropdown.Root>
        </div>
      ...
      <div class="assistant-studio-card__footer">
        <Show when={props.item.publishedRevision}>
          <StatusBadge variant="dot" tone="running" label={t().published} />
        </Show>
        <div class="flex items-center gap-1">
          <Button size="sm" variant="secondary" class="assistant-studio-card__start" onClick={props.onStart}>
          ...
          <Show when={props.external}>
            <IconButtonLink
              size="sm"
              variant="secondary"
              label={locale().startsWith("de") ? "In neuem Tab öffnen" : "Open in new tab"}
              href={`/app/assistant/apps/${props.item.id}`}
              target="_blank"
