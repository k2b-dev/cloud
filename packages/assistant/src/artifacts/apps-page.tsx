...
export default ssr<AuthContext>(async (c) => {
  const user = expectUserBackedActor(c),
    identity = { actor: c.get("actor"), accessSubject: c.get("accessSubject") };
  ...
  try {
    ...
    if (app.permission !== "admin" && !c.req.path.endsWith("/edit") && !c.req.path.endsWith("/database"))
      return c.redirect(`/app/assistant/apps/${app.id}/run`);
    ...
    return () => (
      <Layout c={c} fullPage title={[{ title: t.apps, href: "/app/assistant?studio=1" }, { title: app.title }]}>
        <Apps
          ...
          selectedFile={selectedFile.success ? selectedFile.data : undefined}
        />
      </Layout>
    );
  } catch (e) {
