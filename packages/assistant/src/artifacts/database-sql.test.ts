...
test("resource SELECT accepts bound values and bounds real SQLite results", () => {
  ...
  try {
    ...
    db.transaction(() => {
      for (let i = 0; i < 1010; i++) insert.run("Item " + i, i);
    })();
    expect(db.query(safeQuery("SELECT title, amount FROM records WHERE amount = ?;", 1)).all(42)).toEqual([
      { title: "Item 42", amount: 42 },
    ]);
    ...
    // The namespace is the isolation boundary; its own schema catalog is readable.
    expect(db.query(safeQuery("SELECT name FROM sqlite_master WHERE type = 'table'", 0)).all()).toEqual([{ name: "records" }]);
  } finally {
    db.close();
  }
});

test("resource SQL rejects writes, multiple statements, pragmas and unapproved functions", () => {
  for (const sql of [
    "DELETE FROM records",
    "INSERT INTO records(title) VALUES ('x')",
