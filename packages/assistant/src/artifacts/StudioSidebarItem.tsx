...
function StudioCatalog(props: { open: boolean; close: () => void; activeAppId?: string; modal?: boolean }) {
  ...
  createEffect(() => {
    if (!props.open) pending?.abort();
  });
  onCleanup(() => pending?.abort());
  return (
    <div class="assistant-studio-catalog flex flex-col gap-2">
      <div class="flex items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-1">
          <IconButton
            ...
            onClick={() => {
              props.close();
              openGlobalSearch(assistantAppsSearchOptions(locale()));
            }}
          >
            <i class="ti ti-search" aria-hidden="true" />
          </IconButton>
          <Show when={props.modal}>
            <IconButton size="xs" label={t().closeDialog} onClick={props.close}>
          ...
          <For each={apps()?.items} fallback={<p class="px-2 py-1 text-xs text-dimmed">{t().emptyApps}</p>}>
            {(app) => (
              <AppWorkspace.SidebarItem
                ...
                onClick={props.close}
                active={app.id === props.activeAppId}
              >
      ...
      <Show when={page() > 1 || (!apps.error && apps()?.hasNext)}>
        <div class="flex justify-between gap-2">
          <Button size="sm" variant="ghost" disabled={apps.loading || page() === 1} onClick={() => setPage((value) => value - 1)}>
            {t().back}
          </Button>
          <Button
            size="sm"
            variant="ghost"
            disabled={apps.loading || !!apps.error || !apps()?.hasNext}
...
export function openStudioDialog(options: { activeAppId?: string; signal?: AbortSignal } = {}) {
  return prompts.dialog<void>(
    (close) => (
      <ScrollArea viewportSize="compact">
        <StudioCatalog open close={close} activeAppId={options.activeAppId} modal />
      </ScrollArea>
    ),
    { title: "Studio", header: false, size: "medium", signal: options.signal },
  );
}
  ...
  return (
    <AppWorkspace.SidebarItem
      ...
      preview={{
        ...
        content: (close) => <StudioCatalog open={open()} close={close} activeAppId={props.activeAppId} />,
      }}
    >
      {t().apps}
    </AppWorkspace.SidebarItem>
