...
export async function openArtifactModal(input: unknown, signal: AbortSignal, locale: string) {
  ...
  if (request.kind === "dialog") {
    ...
    // Artifact fields contain scalar values; detach the Solid store before worker RPC.
