...
export const ChatPresentationInput = PresentationFields.superRefine((value, ctx) => {
  if (new TextEncoder().encode(value.code).byteLength > LIMITS.fileBytes) ctx.addIssue({ code: "custom", message: "Source exceeds file byte budget" });
  ...
  try { validateTree(value.nodes); } catch (error) { ctx.addIssue({ code: "custom", message: String(error) }); }
});
