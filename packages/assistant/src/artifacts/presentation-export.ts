...
const escape = (value: unknown) =>
  String(value ?? "").replace(/[&<>"']/g, (value) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[value]!);
...
export function presentationHtml(nodes: AnalyticsNode[], title: string, locale: string) {
  validateTree(nodes);
  if (nodes.some((node) => node.loading || (node.type === "group" && node.error)))
    throw new Error(artifactMessages.resolve([locale]).t.visualizationWait);
  ...
  function render(node: AnalyticsNode): string {
    ...
    switch (node.type) {
      case "button":
        return "";
      case "layout":
        body = `<div class="${node.layout === "grid" || node.layout === "row" ? "grid" : "column"}">${node.children.map((id) => render(by.get(id)!)).join("")}</div>`;
        break;
      case "text":
        body = `<p class="text">${escape(node.value)}</p>`;
        break;
      case "stat":
        body = `<p class="stat">${escape(formatValue(node.value, node.format, locale))}</p>`;
        break;
      case "chart":
        body = presentationChartSvg(node, locale);
        break;
      case "table":
      case "explorer": {
        ...
        if (node.type === "explorer") {
          ...
          if (context)
            body += `<p>${escape(context.mode)} · ${escape(context.asOf)} · ${escape(context.status)}<br>${context.sources.map((source) => escape(source.label)).join(" · ")}<br>${escape(context.note)}</p>`;
          if (node.view === "chart") body += presentationChartSvg(node, locale);
        }
        if (node.type === "table" || node.view === "table")
          body += `<table><thead><tr>${node.columns.map((column) => `<th>${escape(column.label)}</th>`).join("")}</tr></thead><tbody>${rows.map((row) => `<tr>${node.columns.map((column) => `<td>${escape(formatValue(row[column.key] ?? null, column.format, locale))}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
        break;
      }
      case "select":
        body = `<p>${escape(node.options.find((option) => option.value === node.value)?.label ?? node.value)}</p>`;
        break;
      case "multiSelect":
        body = `<p>${node.value.map((value) => escape(node.options.find((option) => option.value === value)?.label ?? value)).join(", ")}</p>`;
        break;
      case "dateRange":
        body = `<p>${escape(node.value.start)} – ${escape(node.value.end)}</p>`;
        break;
      case "filePicker":
        body = `<p>${escape(node.names)}</p>`;
        break;
      case "group": {
      ...
      case "number":
      case "slider":
        body = `<p>${escape(node.value)}</p>`;
        break;
      default: {
        const unsupported: never = node;
        throw new Error(`Unsupported export node: ${unsupported}`);
      }
    }
...
</style></head><body><h1>${escape(title)}</h1>${nodes
