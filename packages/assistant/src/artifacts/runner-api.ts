...
export async function compiledRunner(id: string, identity: Parameters<typeof artifacts.runner>[1]) {
  ...
  if (current.sourceRevision !== bundle.sourceRevision || current.serverAccess !== bundle.serverAccess) throw new ArtifactError("CONFLICT");
  ...
  new Hono<AuthContext>()
    ...
    .onError((error, c) => {
      if (error instanceof ArtifactCompileError)
        return respond(c, { ok: false, code: "COMPILE_FAILED", status: 400, error: compilationDiagnostic(error) });
