...
export const ArtifactFile = z.object({
  ...
  content: z.string().max(LIMITS.fileBytes).describe("Complete UTF-8 source file content. Read an existing file completely before replacing it."),
}).strict();
export const ArtifactSource = z.object({
  ...
  if (!/\.(?:js|ts)$/.test(source.entry))
    ctx.addIssue({ code: "custom", message: "Entry must be a JavaScript or TypeScript path" });
