...
import { STORAGE_FILE_MAX_BYTES, StorageRequest } from "./storage-contracts";
...
type ArtifactRow = {
  ...
  short_id: string;
  forked_from_short_id?: string | null;
  kind: ArtifactKind;
  icon: string;
...
const summarize = (row: ArtifactRow, permission: PermissionLevel): ArtifactSummary => ({
  kind: row.kind,
  icon: permission === "admin" ? row.icon : (row.published_icon ?? row.icon),
  publishedVersion: row.published_version,
  id: row.short_id,
  title: permission === "admin" ? row.title : (row.published_title ?? row.title),
  description: (permission === "admin" ? row.description : row.published_description) ?? "",
  revision: permission === "admin" ? row.revision : row.published_revision!,
  permission,
  updatedAt: row.updated_at.toISOString(),
  publishedRevision: row.published_revision,
  forkedFromId: row.forked_from_short_id ?? null,
...
async function accessibleLinkedProjects(db: SQL, subject: AccessSubject, artifactId?: string) {
  ...
  return [
    ...(
      await aiProjects.resolveShortIds(
...
async function revision(db: SQL, row: ArtifactRow, permission: PermissionLevel, sourceRevision = row.revision): Promise<ArtifactBundle> {
  ...
  const [origin] = row.forked_from_id
    ? await db<{ short_id: string }[]>`SELECT short_id FROM assistant.artifacts WHERE id=${row.forked_from_id}::uuid`
    : [];
...
async function managementState(db: SQL, row: ArtifactRow) {
  ...
  const grants = await db<
    { access_id: string }[]
  >`SELECT access_id FROM assistant.artifact_access WHERE artifact_id=${row.id}::uuid ORDER BY access_id`;
  const entries = await Promise.all(grants.map((grant) => getAccess({ id: grant.access_id }, db)));
  const projects = await db<
    { project_id: string }[]
  >`SELECT project_id FROM assistant.artifact_projects WHERE artifact_id=${row.id}::uuid ORDER BY project_id`;
  const state = {
    id: row.short_id,
    title: row.title,
  ...
  const managementRevision = new Bun.CryptoHasher("sha256")
    .update(
      JSON.stringify({
        state,
        ...
        database,
        projects,
        access: accessRevision(entries.filter((entry) => entry !== null)),
      }),
    )
    .digest("hex");
...
export const artifacts = {
  ...
  async runner(id: string, identity: { actor?: ArtifactIdentity["actor"]; accessSubject?: AccessSubject }) {
    ...
    return sql.begin(async (db) => {
      ...
      if (identity.actor && userFromActor(identity.actor) && identity.accessSubject?.type === "user") {
        try {
          authorized = await requireArtifact(db, id, { actor: identity.actor, accessSubject: identity.accessSubject }, "read");
        } catch (error) {
  ...
  async remove(id: string, identity: ArtifactIdentity, expectedManagementRevision?: string) {
    return sql.begin(async (db) => {
      ...
      id = row.id;
      if (expectedManagementRevision !== undefined && (await managementState(db, row)).managementRevision !== expectedManagementRevision)
  ...
  ): Promise<Array<ArtifactSummary & { description: string; icon: string }>> {
  ...
  async list(identity: ArtifactIdentity, page = 1, search = "", pageSize = 30) {
    ...
    const rows = await sql<
      (ArtifactRow & { permission: PermissionLevel })[]
    >`SELECT p.*, (SELECT origin.short_id FROM assistant.artifacts origin WHERE origin.id=p.forked_from_id) AS forked_from_short_id,
  ...
  async writeFiles(
    id: string,
    input: { expectedRevision: number; entry?: string; files: { path: string; content: string }[] },
  ...
  async history(id: string, identity: ArtifactIdentity, page = 1) {
    ...
    return sql.begin(async (db) => {
      ...
      return {
        items: rows.slice(0, 30).map((row) => ({ revision: row.revision, createdAt: row.created_at.toISOString() })),
  ...
  async publish(id: string, expectedRevision: number, identity: ArtifactIdentity, note: string) {
    ...
    return sql.begin(async (db) => {
      ...
      const [next] = await db<
        { version: number }[]
      >`SELECT coalesce(max(version),0)+1 AS version FROM assistant.artifact_publications WHERE artifact_id=${id}::uuid`;
  ...
  async storageState(id: string, identity: ArtifactIdentity) {
    return sql.begin(async (db) => {
      ...
      return {
        ...
        title: row.title,
        storageRevision: Number(row.storage_revision),
        areas: areas.map((area) => ({ ...area, bytes: Number(area.bytes) })),
      };
    });
  },
  ...
  async storageFileLimit(id: string, key: string, identity: ArtifactIdentity, management = false) {
    ...
    const maximum = Math.min(
      ...
      Math.max(previous, Number(await app.settings.get("assistant.storage_file_mib")) * 1024 * 1024),
    );
  ...
  async storage(
    ...
    const limits =
      request.operation === "write" && request.area === "files"
        ? {
      ...
      if (request.operation === "write") {
        ...
        if (
          !checkStorageBudget({
            area: request.area,
            bytes,
            total: Number(usage!.bytes),
            previous: Number(usage!.previous),
            items: Number(usage!.items),
  ...
  async projects(id: string, identity: ArtifactIdentity) {
    return sql.begin(async (db) => {
      ...
      const links = await db<
        { projectId: string }[]
      >`SELECT project_id AS "projectId" FROM assistant.artifact_projects WHERE artifact_id=${id}::uuid ORDER BY project_id`;
  ...
  async linkProject(id: string, projectReference: string, linked: boolean, identity: ArtifactIdentity) {
    ...
    return sql.begin(async (db) => {
      ...
      id = row.id;
      if (linked)
        await db`INSERT INTO assistant.artifact_projects(artifact_id,project_id) VALUES(${id}::uuid,${projectId}::uuid) ON CONFLICT DO NOTHING`;
  ...
  async editChat(id: string, identity: ArtifactIdentity, prompt?: string) {
    ...
    const conversation = await aiConversations.createConversation({
      ...
      title: bundle.title,
      draft: [
        { type: "resource", ref, title: bundle.title, icon: "ti ti-app-window", href },
  ...
  async fork(id: string, identity: ArtifactIdentity) {
    ...
    return sql.begin(async (db) => {
      ...
      const [copy] = await withAiShortIdForDb(
        ...
        (tx, shortId) => tx<
          ArtifactRow[]
        >`INSERT INTO assistant.artifacts(short_id,kind,title,description,icon,forked_from_id,forked_from_revision)
