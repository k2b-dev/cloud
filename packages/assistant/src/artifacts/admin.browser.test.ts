...
test("Studio administration renders inventory and tests settings without saving secrets", async () => {
  const build=Bun.spawn(["bun",new URL("./workspace-browser-build.ts",import.meta.url).pathname,"./admin-browser-harness.tsx"],{stdout:"pipe",stderr:"pipe"});
  ...
  const writes:unknown[]=[],tests:unknown[]=[];
  const server=Bun.serve({hostname:"127.0.0.1",port:0,async fetch(request){
    ...
    if(path.endsWith("/database/settings")){
      if(request.method==="PUT"){writes.push(await request.json());return Response.json({url:"http://database.test",tokenSet:true});}
      return Response.json({url:"http://database.test",tokenSet:true});
    }
    if(path.endsWith("/database/test")){tests.push(await request.json());return Response.json({connected:true});}
    if(path.endsWith("/access"))return Response.json([]);
    return new Response('<!doctype html><link rel="stylesheet" href="/ui.css"><body class="k2b-ui"><div id="root"></div><script src="/bundle.js"></script>',{headers:{"Content-Type":"text/html"}});
  }});
