...
test("PDF multipart roundtrip preserves binary bytes, types and metadata", async () => {
  const source = {
    operation: "attach",
    document: new Blob(["%PDF-document"]),
    attachments: [
      { name: "invoice.xml", data: new Blob([new Uint8Array([0, 255, 17])], { type: "application/xml" }), relationship: "Data" },
    ],
  };
...
test("PDF rejects unsupported options, asset collisions and configured byte overflows", async () => {
  await expect(
    executePdf({ operation: "render", html: "<script>" + "x".repeat(2048) + "</script>" }, { ...config, maxHtmlBytes: 1024 }),
  ...
  expect(
    PdfRequest.safeParse({ operation: "attach", document: new Blob(), attachments: [{ name: "../x", data: new Blob() }] }).success,
  ).toBe(false);
  await expect(
    executePdf({ operation: "render", html: "hi", assets: [{ name: "index.html", data: new Blob() }] }, config),
  ).rejects.toMatchObject({ code: "bad_input" });
  await expect(
    executePdf(
      { operation: "attach", document: new Blob(["12345"]), attachments: [{ name: "x.xml", data: new Blob(["67890"]) }] },
      { ...config, maxPdfBytes: 9 },
    ),
...
test("PDF output is bounded while streaming and cancels upstream on overflow", async () => {
  ...
  await expect(
    executePdf(
      { operation: "render", html: "hi" },
      { ...config, maxPdfBytes: 4 },
      {
        fetch: async () =>
          new Response(
            new ReadableStream({
...
test("runtime PDF authorizes Use access without asking for Manage", async () => {
  ...
  const get = spyOn(artifacts, "get").mockResolvedValue({
    id: "ABC234",
    kind: "app",
    title: "Shared app",
    permission: "read",
    ...
    source: { entry: "main.js", files: [{ path: "main.js", content: "export default()=>{}" }] },
  });
  try {
...
test("runtime PDF denies before parsing or executing and returns binary PDF on success", async () => {
  ...
  const execute = spyOn(studioPdf, "execute").mockResolvedValue({
    pdf: new TextEncoder().encode("%PDF-output"),
  ...
  try {
    ...
    const response = await routes.request("/runtime/pdf?resourceId=ABC234", {
      method: "POST",
      body: encodePdfRequest({ operation: "render", html: "hello" }),
    });
