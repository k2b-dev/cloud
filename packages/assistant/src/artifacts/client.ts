import type { CodeActionInput } from "@k2b/cloud/ai/browser";
...
export const artifactClient = {
  ...
  projectApps: async (projectId: string, page = 1, q = "", available = false, signal?: AbortSignal) => {
    const response = await client["project-links"][":projectId"].$get(
  ...
  pdf: async (input: unknown, scope: HttpScope, signal?: AbortSignal): Promise<Blob> => {
    ...
    const response = await fetch(`/api/assistant/artifacts/runtime/pdf?${query}`, {
      method: "POST",
      body: encodePdfRequest(input),
      signal,
    });
    await checked(response);
    return response.blob();
  },
  secrets: async (scope: HttpScope) => {
    const response = await client.runtime.secrets.list.$post({ json: scope });
    await checked(response);
    return z.array(SecretView).parse(await response.json());
  },
  saveSecret: async (scope: HttpScope, secret: z.infer<typeof SecretSave>) => {
    const response = await client.runtime.secrets.$put({ json: { scope, secret } });
    await checked(response);
    return SecretView.parse(await response.json());
  },
  removeSecret: async (scope: HttpScope, name: string, revision: string) => {
    const response = await client.runtime.secrets.remove.$post({ json: { scope, name, revision } });
    await checked(response);
  },
  httpPrepare: async (input: HttpPrepare, signal?: AbortSignal) => {
    const response = await client.runtime.http.$post({ json: input }, { init: { signal } });
    await checked(response);
  ...
  httpExecute: async (id: string, approved: boolean, signal?: AbortSignal) => {
    ...
    await checked(response);
    return HttpResult.parse(await response.json());
  },
  databaseInspect: async (id: string, input: unknown, signal?: AbortSignal) => {
    const response = await client[":id"].database.inspect.$post(
      { param: { id }, json: DatabaseRequest.parse(input) },
      { init: { signal } },
    );
    await checked(response);
    return response.json();
  },
  renameSource: async (source: ArtifactSource, from: string, to: string) => {
    const response = await client.runtime.rename.$post({ json: { source, from, to } });
    await checked(response);
    return ArtifactSource.parse(await response.json());
  },
  databaseStatus: async (id: string) => {
    const response = await client[":id"].database.status.$get({ param: { id } });
    await checked(response);
    return response.json();
  },
  databaseReset: async (id: string, expectedGeneration: string | null) => {
    const response = await client[":id"].database.reset.$post({ param: { id }, json: { confirmed: true, expectedGeneration } });
    await checked(response);
    return response.json();
  },
  ...
  storageFile: async (id: string, key: string, options: { data?: Blob; conversationId?: string; management?: boolean } = {}) => {
    ...
    const response = await fetch(`/api/assistant/artifacts/${encodeURIComponent(id)}/storage/file?${query}`, {
      method: options.data === undefined ? "GET" : "PUT",
      body: options.data,
  ...
  remove: async (id: string) => {
    const response = await client[":id"].$delete({ param: { id } });
    await checked(response);
    return response.json();
  },
  capabilityPrepare: async (
    input: { id: string; name: string; input: unknown; artifactId?: string; conversationId?: string },
    signal?: AbortSignal,
  ) => {
    const response = await client.runtime.capabilities.$post(
      { json: { ...input, input: z.json().parse(input.input) } },
      { init: { signal } },
    );
    await checked(response);
    return response.json();
  ...
  capabilityResolve: async (id: string, input: { approved: boolean; remember?: "always" }, signal?: AbortSignal) => {
    const response = await client.runtime.capabilities[":callId"].resolve.$post(
      { param: { callId: id }, json: input },
      { init: { signal } },
    );
    await checked(response);
    return response.json();
  ...
  projects: async (id: string) => {
    const response = await client[":id"].projects.$get({ param: { id } });
    await checked(response);
    return response.json();
  },
  linkProject: async (id: string, projectId: string, linked: boolean) => {
    const response = await client[":id"].projects[":projectId"].$put({ param: { id, projectId }, json: { linked } });
    await checked(response);
    return response.json();
  },
  admin: {
    projects: async (id: string) => {
      const response = await client.admin.resources[":id"].projects.$get({ param: { id } });
      await checked(response);
      return response.json();
    },
    remove: async (id: string) => {
      const response = await client.admin.resources[":id"].$delete({ param: { id } });
      await checked(response);
      return response.json();
    },
    settings: async () => {
      const response = await client.admin.database.settings.$get();
      await checked(response);
      return response.json();
    },
    configure: async (input: { url: string; token?: string }) => {
      const response = await client.admin.database.settings.$put({ json: input });
      await checked(response);
      return response.json();
    },
    test: async (input: { url: string; token?: string }) => {
      const response = await client.admin.database.test.$post({ json: input });
      await checked(response);
      return response.json();
    },
    access: async (id: string) => {
      const response = await client.admin.resources[":id"].access.$get({ param: { id } });
      await checked(response);
      return response.json();
    },
    grant: async (
      id: string,
      principal: { type: "user"; userId: string } | { type: "group"; groupId: string } | { type: "authenticated" } | { type: "public" },
      permission: "read" | "admin",
    ) => {
      const response = await client.admin.resources[":id"].access.$post({ param: { id }, json: { principal, permission } });
      await checked(response);
      return response.json();
    },
    change: async (id: string, accessId: string, permission: "read" | "admin" | null) => {
      const response = await client.admin.resources[":id"].access[":accessId"].$put({ param: { id, accessId }, json: { permission } });
      await checked(response);
      return response.json();
    },
  },
  database: async (id: string, request: unknown, conversationId?: string, signal?: AbortSignal, management = false) => {
    ...
    const response = await fetch(
      `/api/assistant/artifacts/${encodeURIComponent(id)}/database${management ? "/maintenance" : ""}${connect ? "/connect" : ""}${conversationId ? "?conversationId=" + encodeURIComponent(conversationId) : ""}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  get: async (id: string, published = false, version?: number, conversationId?: string) => {
    const response = await client[":id"].$get({
      param: { id },
      query: { conversationId, ...(published ? { published: "true" as const } : {}), ...(version ? { version: String(version) } : {}) },
    });
    await checked(response);
    return response.json();
  },
