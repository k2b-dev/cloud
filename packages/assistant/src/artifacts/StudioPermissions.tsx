...
export function StudioPermissions(props: {
  ...
  return <div class="flex flex-col gap-3">
    <Show when={projects.loading}><InlineGuidance loading>{t().loading}</InlineGuidance></Show>
    <Show when={projects.error}><InlineGuidance tone="danger">{t().loadFailed}
      <Button size="sm" variant="ghost" onClick={() => void refetch()}>{t().retry}</Button>
    </InlineGuidance></Show>
    <Show when={!projects.error && projects()?.length}>
      <NoticeCard tone="info" title={t().projectAccessTitle} detail={t().projectScriptsHelp}>
        <ul class="flex flex-col gap-1"><For each={projects()}>{project => <li class="flex items-center gap-2 text-sm">
          <i class="ti ti-folders" aria-hidden="true" />{project.name ?? t().inaccessibleProject}
        </li>}</For></ul>
      </NoticeCard>
    </Show>
