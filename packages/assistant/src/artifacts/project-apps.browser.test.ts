...
for (const kind of ["apps","skills"] as const) test(`project ${kind} can be searched, linked and unlinked while readers have no controls`, async () => {
  ...
  const notice = kind === "apps" ? "Members can use linked, published apps and their shared app data. Editing permissions stay unchanged." : "Members can read and use linked Skills. Editing permissions stay unchanged.";
  const build = Bun.spawn(["bun", new URL("./workspace-browser-build.ts", import.meta.url).pathname, "./project-apps-browser-harness.tsx"], {stdout:"pipe",stderr:"pipe"});
  ...
  const server = Bun.serve({hostname:"127.0.0.1",port:0,async fetch(request) {
    ...
    if (url.pathname.endsWith("/project-links/project1")) {
      ...
      const item = kind === "apps" ? {id:"app123",title:"Bank reconciliation",icon:"ti ti-app-window",published:false,canManage:true,linked} :
        {id:"app123",shortId:"app123",name:"Bank reconciliation",description:"Reconcile statements.",permission:"admin",enabled:true,revision:1,referenceCount:0,createdAt:"",updatedAt:""};
      return Response.json({items: available !== linked ? [item] : [],page:1,hasNext:false});
    }
    if (url.pathname.endsWith("/app123/projects/project1")) {
      linked = (await request.json()).linked; writes.push(linked); return Response.json({linked});
    }
    return new Response('<!doctype html><meta charset="utf-8"><link rel="stylesheet" href="/ui.css"><body class="k2b-ui"><div id="root"></div><script src="/bundle.js"></script>',{headers:{"content-type":"text/html"}});
  }});
  ...
  try {
    ...
    await Promise.all([page.waitForResponse(response => response.url().includes("q=Bank")), page.getByPlaceholder(searchLabel).fill("Bank")]);
    await page.getByText("Bank reconciliation",{exact:true}).click();
    await page.getByRole("dialog").waitFor({state:"hidden"});
    await page.getByText(kind === "apps" ? "Draft — publish before members can use it." : "Reconcile statements.",{exact:true}).waitFor();
    expect(writes).toEqual([false,true]); expect(searches).toContain("Bank");
    await page.getByRole("button",{name:"Unlink · Bank reconciliation",exact:true}).click();
    await page.getByText(empty,{exact:true}).waitFor();
