...
export const artifactDatabase = {
  async status(id: string, identity: ArtifactIdentity, signal?: AbortSignal) {
    return sql.begin(async (db) => {
      ...
      const [mapping] = await db<
        { namespace: string; connected: boolean; data_revision: string }[]
      >`SELECT namespace,connected,data_revision FROM assistant.artifact_databases WHERE artifact_id=${id}::uuid`;
      const configured = Boolean(c.url && c.token);
      let overview = null,
        unavailable: string | null = null;
      if (configured && mapping?.connected) {
        try {
          overview = result(await connection(c, signal).ns(mapping.namespace).overview.get());
        } catch (error) {
          if (signal?.aborted) throw error;
          unavailable = error instanceof DatabaseError ? error.code : "DB_UNREACHABLE";
        }
      }
      return {
        configured,
        connected: mapping?.connected ?? false,
        generation: mapping ? generation(mapping.namespace) : null,
        dataRevision: mapping?.data_revision ?? null,
        overview,
        unavailable,
      };
    });
  ...
  async reset(id: string, expectedGeneration: string | null, identity: ArtifactIdentity, expectedDataRevision?: string | null) {
    return sql.begin(async (db) => {
      ...
      id = (await requireArtifact(db, id, identity, "admin")).row.id;
      const [mapping] = await db<
        { namespace: string; data_revision: string }[]
      >`SELECT namespace,data_revision FROM assistant.artifact_databases WHERE artifact_id=${id}::uuid`;
      if (
        (mapping && generation(mapping.namespace) !== expectedGeneration) ||
        (expectedDataRevision !== undefined && (mapping?.data_revision ?? null) !== expectedDataRevision)
      )
        throw new DatabaseError("CONFLICT", 409);
  ...
  async clear(id: string, expectedGeneration: string, expectedDataRevision: string, identity: ArtifactIdentity, signal?: AbortSignal) {
    ...
    return sql.begin(async (db) => {
      ...
      if (generation(mapping.namespace) !== expectedGeneration || mapping.data_revision !== expectedDataRevision)
        throw new DatabaseError("CONFLICT", 409);
          ...
          return {
            ...
            error: signal.aborted
              ...
              : error instanceof DatabaseError
                ? error.code
                : "DB_UNREACHABLE",
            outcome: "The failed table may have changed; inspect before retrying.",
          };
        }
      }
  ...
  async settings(identity: ArtifactIdentity) {
    admin(identity);
    const c = await config();
    return { url: c.url, tokenSet: Boolean(c.token) };
  },
  async configure(input: unknown, identity: ArtifactIdentity, testOnly = false) {
    ...
    return sql.begin(async (db) => {
      ...
      if (!testOnly && (next.url !== current.url || !token)) {
        const [used] = await db<
          { count: number }[]
        >`SELECT ((SELECT count(*) FROM assistant.artifact_databases)+(SELECT count(*) FROM assistant.database_cleanup))::int AS count`;
  ...
  async connect(id: string, identity: ArtifactIdentity, signal?: AbortSignal, requireManage = false) {
    ...
    return sql.begin(async (db) => {
      ...
      const [mapping] = await db<
        { namespace: string; connected: boolean }[]
      >`SELECT * FROM assistant.artifact_databases WHERE artifact_id=${id}::uuid`;
  ...
  async call(
    ...
    return sql.begin(async (db) => {
      ...
      if (["tables.create", "tables.update", "tables.delete", "rows.insert", "rows.update", "rows.delete"].includes(req.operation))
        await markDataMutation(id);
      let data: unknown;
  ...
  async cleanup() {
    return sql.begin(async (db) => {
      ...
      const [item] = await db<
        { namespace: string }[]
      >`SELECT namespace FROM assistant.database_cleanup ORDER BY created_at LIMIT 1 FOR UPDATE SKIP LOCKED`;
      if (!item) return;
