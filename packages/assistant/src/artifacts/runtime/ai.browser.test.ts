...
test("opaque worker awaits AI beyond the startup watchdog and cancels server requests on stop", async () => {
  const build = Bun.spawn(["bun", "build", new URL("./ai-browser-harness.ts", import.meta.url).pathname, "--target", "browser", "--format", "iife"], { stdout: "pipe", stderr: "pipe" });
  ...
  const source = await compileArtifact({ entry: "main.js", files: [{ path: "main.js", content: `export default async () => ({
  ...
  const server = Bun.serve({ port: 0, hostname: "127.0.0.1", async fetch(request) {
    if (new URL(request.url).pathname !== "/ai") return new Response("<!doctype html><body></body>", { headers: { "Content-Type": "text/html" } });
    const input = AiTaskRequestSchema.parse(await request.json());
    calls++;
    if (calls === 1) await Bun.sleep(16000);
    return Response.json(input.kind === "generate_text" ? "Summary" : input.kind === "classify" ? "a" : input.kind === "classify_many" ? ["a"] : { ready: true });
  } });
  ...
  try {
    ...
    expect(await page.evaluate(source => window.runAiScenario(source, false), source)).toEqual({ text: "Summary", category: "a", categories: ["a"], data: { ready: true } });
    expect(calls).toBe(4);
