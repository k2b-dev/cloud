...
export function createArtifactSession(container: HTMLElement, source: { runtime: string; code: string }, options: SessionOptions) {
  ...
  const run = startArtifactRun(container, source, {
    ...
    work: (work) => {
      clearTimeout(watchdog);
      emit({ work });
      if (work.status !== "running" && state.status === "starting" && !state.inputPending) arm();
    },
    busy: (busy) => emit({ busy }),
    log,
    ...
    request: async (method, args, signal) => {
      ...
      if (method === "file.open" || method === "file.openMultiple" || method === "file.openFolder") {
        if (options.mode === "test") {
          ...
          try {
            const selected =
              typeof options.pickerInputs === "function" ? await options.pickerInputs(signal) : (options.pickerInputs ?? inputs);
        ...
        const accept =
          typeof args[0] === "object" && args[0] !== null && "accept" in args[0] && typeof args[0].accept === "string"
            ? args[0].accept
            : "";
        const previous = state.status;
        clearTimeout(watchdog);
      ...
      if (method === "capabilities.run") {
        ...
        try {
          ...
          capabilityRequests--;
          emit({ approvalPending: capabilityRequests > 0 });
          if (!capabilityRequests && state.status === "starting" && !signal.aborted) arm();
        }
      }
      if (method === "capabilities.stream") {
        ...
        emit({ inputPending: true });
        try {
          return await runStream(ref, verb, args[2] instanceof Blob ? args[2] : undefined, signal);
        } finally {
          streamRequests--;
          emit({ inputPending: streamRequests > 0 });
          if (state.status === "starting" && !signal.aborted) arm();
        }
      }
      if (method === "http.fetch") {
        ...
        emit({ approvalPending: true });
        try {
          return await options.http(args[0], signal);
        } finally {
          capabilityRequests--;
          emit({ approvalPending: capabilityRequests > 0 });
          if (!capabilityRequests && state.status === "starting" && !signal.aborted) arm();
      ...
      if (method === "ai") {
        ...
        emit({ inputPending: true });
        try {
          return await options.ai(args[0], signal);
        } finally {
          capabilityRequests--;
          emit({ inputPending: false });
          if (state.status === "starting" && !signal.aborted) arm();
      ...
      if (method === "pdf") {
        ...
        emit({ inputPending: true });
        try {
          return await options.pdf(args[0], signal);
        } finally {
          emit({ inputPending: false });
          if (state.status === "starting" && !signal.aborted) arm();
        }
  ...
  return {
    ...
    event: async (event: RuntimeEvent) => {
      ...
      if (state.status === "error" && previousError === errorGeneration) emit({ status: "ready", error: undefined });
    },
