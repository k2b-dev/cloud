...
export const LinkOptions = z.object({
  href: z
    ...
    .refine((href) => {
      try {
        return ["https:", "http:", "mailto:"].includes(new URL(href, "https://artifact.invalid/").protocol);
      } catch {
        return false;
      }
    ...
    items: z
      .array(
        z.object({
          ...
          description: z.string().max(LIMITS.text).default(""),
          icon: z.string().max(120).default(""),
          data: z.record(z.string().max(120), Value).optional(),
        }),
      )
      .max(LIMITS.rows)
      .default([]),
    actions: z.array(z.object({
      id: z.string().min(1).max(80), label: z.string().min(1).max(120),
      icon: z.string().max(120).optional(),
      variant: z.enum(["primary", "secondary", "ghost", "text", "danger"]).optional(),
