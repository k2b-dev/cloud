    ...
    if ("kind" in chart) {
      ...
      if (chart.kind === "line") {
        ...
        if (
          ...
          ctx.addIssue({
            code: "custom",
            message: `Line field ${chart.x} requires either finite numbers or nonempty category labels, without mixing types`,
          });
      }
  ...
  .discriminatedUnion("type", [
    ...
    z
      .object({
        ...
        format: Format.optional(),
        trend: z.array(z.number().finite()).max(1000).optional(),
      })
      .strict(),
    z
      .object({
