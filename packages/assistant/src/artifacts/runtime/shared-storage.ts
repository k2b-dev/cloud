...
export function localStorageCall(request: z.infer<typeof RuntimeStorage>) {
  ...
  return {method:operations[request.operation],args:[request.key,request.value]};
}
...
export async function sharedStorage(id: string, request: z.infer<typeof RuntimeStorage>, conversationId?: string) {
  ...
  const result = await artifactClient.storage(id,{area:request.area,operation:request.operation,key:request.key,content,mediaType},conversationId);
