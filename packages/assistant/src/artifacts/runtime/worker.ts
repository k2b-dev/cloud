...
import { camt, datev, einvoice, sepa } from "@k2b/stdlib/finance";
...
function scopedStorage(scope: "local" | "shared", area: "kv" | "files") {
  ...
  return area === "kv"
    ? {
        get: (key: string) => call("read", key),
        set: (key: string, value: unknown) => call("write", key, value),
        delete: (key: string) => call("delete", key),
        keys: list,
      }
    : {
        read: (key: string) => call("read", key),
        write: (key: string, value: Blob | string) => call("write", key, value),
        delete: (key: string) => call("delete", key),
        list,
      };
}
...
const api = {
  ...
  files: {
    local: scopedStorage("local", "files"),
    shared: scopedStorage("shared", "files"),
    list: () => rpc("file.list"),
  ...
  work: createWork(
    (state) => send({ type: "work", ...state }),
    sendOutput,
    (error) => send({ type: "error", text: textError(error).slice(0, LIMITS.text) }),
  ),
  pdf: createPdf(rpc),
  sheet: {
    ...
    fromCsv: async (file: File | string, options: { delimiter?: string; encoding?: string } = {}) => {
      ...
      try {
        content =
          typeof file === "string" ? file : new TextDecoder(options.encoding ?? "utf-8", { fatal: true }).decode(await file.arrayBuffer());
      } catch {
        throw new Error("CSV decoding failed. Specify encoding, for example windows-1252 for older Excel exports.");
      }
