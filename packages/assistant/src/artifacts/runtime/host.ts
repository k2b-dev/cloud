...
export function startArtifactRun(container: HTMLElement, source: { runtime: string; code: string }, hooks: RuntimeHooks) {
  ...
  const events = new Map<
    number,
    { resolve: () => void; reject: (error: Error) => void; timer?: ReturnType<typeof setTimeout>; expire: () => void; timeoutMs: number }
  >();
  ...
  function receive(event: MessageEvent) {
    ...
    try {
      ...
      if (m.type === "cancel") {
        requests.get(m.id)?.abort();
      } else if (m.type === "ui") {
        validateTree(m.nodes);
        hooks.ui(m.nodes);
        ...
        if (m.status === "running") {
          ...
          if (!waitingForModal)
            for (const event of events.values()) {
              clearTimeout(event.timer);
              event.timer = setTimeout(event.expire, event.timeoutMs);
            }
        }
        const { type, ...state } = m;
        hooks.work?.(state);
        ...
        if (pending) {
          ...
          events.delete(m.id);
          hooks.busy(events.size > 0);
          m.error ? pending.reject(new Error(m.error)) : pending.resolve();
        }
      } else if (m.type === "rpc") {
