...
test("only instance administrators configure validated storage settings",async()=>{
  ...
  const read=spyOn(app.settings,"get").mockImplementation(async key=>({
    "assistant.storage_file_mib":50,"assistant.storage_total_mib":250,
    "assistant.rsql_url":"","assistant.rsql_api_token":"",
  }[key]));
  ...
  try{
    ...
    for(const value of [{fileMiB:0,totalMiB:250},{fileMiB:65,totalMiB:250},{fileMiB:50,totalMiB:1.5}]){
