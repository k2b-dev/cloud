...
const Name = z.string().min(1).max(180).refine(name => !/[\\/\x00-\x1f]/.test(name) && name !== "." && name !== "..", "Use a plain filename without directories");
const Data = z.instanceof(Blob);
const Asset = z.object({ name: Name, data: Data }).strict();
const Margin = z.number().finite().nonnegative();
export const PdfHtml = z.object({
  html: z.string().min(1),
  ...
  page: z.object({
    ...
    margin: z.object({ top: Margin.optional(), right: Margin.optional(), bottom: Margin.optional(), left: Margin.optional() }).strict().optional(),
  }).strict().default({ format: "A4", landscape: false }),
  tagged: z.boolean().default(true),
}).strict();
export const PdfAttachment = Asset.extend({ relationship: z.enum(["Source", "Data", "Alternative", "Supplement", "Unspecified"]).default("Unspecified") });
export const PdfRequest = z.discriminatedUnion("operation", [
  PdfHtml.extend({ operation: z.literal("render") }),
  PdfHtml.extend({ operation: z.literal("facturX"), xml: z.string().min(1), profile: z.enum(["MINIMUM", "BASIC WL", "BASIC", "EN 16931", "EXTENDED"]) }),
...
export function checkPdfBytes(request: PdfRequest) {
  const size = request.operation === "attach"
    ? request.document.size + request.attachments.reduce((sum, file) => sum + file.data.size, 0)
    : new Blob([request.html, request.headerHtml ?? "", request.footerHtml ?? "", request.operation === "facturX" ? request.xml : ""]).size
      + request.assets.reduce((sum, file) => sum + file.data.size, 0);
...
export function encodePdfRequest(input: unknown): FormData {
  ...
  if (request.operation === "attach") {
    ...
    form.append("request", JSON.stringify({ ...request, document: undefined, attachments: request.attachments.map(({ data, ...metadata }) => ({ ...metadata, type: data.type })) }));
  } else {
    request.assets.forEach(file => form.append("files", file.data, file.name));
    form.append("request", JSON.stringify({ ...request, assets: request.assets.map(({ data, ...metadata }) => ({ ...metadata, type: data.type })) }));
  }
  return form;
}

export function decodePdfRequest(form: FormData): PdfRequest {
  const metadata = z.object({ operation: z.enum(["render", "facturX", "attach"]) }).passthrough().parse(JSON.parse(z.string().parse(form.get("request"))));
  ...
  const request = PdfRequest.parse({ ...metadata, [key]: values, ...(metadata.operation === "attach" ? { document: form.get("document") } : {}) });
  checkPdfBytes(request);
  return request;
}
