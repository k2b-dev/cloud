...
import { type AuthContext, accessRevision } from "@k2b/cloud/server";
...
databaseSuite()("Assistant artifacts in disposable Postgres", () => {
  ...
  test("public runner exposes only the publication and never grants server or management access", async () => {
    ...
    await expect(artifacts.storage(resource.id, { area: "kv", operation: "read", key: "secret" }, stranger)).rejects.toMatchObject({
      code: "ACCESS_DENIED",
    });
  ...
  test("published actions are discoverable with Use, validate inputs and reject stale releases", async () => {
    ...
    const api = new Hono<AuthContext>()
      .use("*", async (c, next) => {
        c.set("actor", reader.actor);
        c.set("accessSubject", reader.accessSubject);
        await next();
      })
      .route("/", createArtifactServiceRoutes());
    const call = (input: unknown, publishedVersion = 1) =>
      api.request("/runtime/action", {
        method: "POST",
    ...
    try {
      ...
      expect(JSON.stringify(await artifactCodeHandlers.code_actions({ id: resource.id, draft: false }, context))).not.toContain(
        '"revision"',
      );
      expect(await artifactCodeHandlers.code_actions({ id: resource.id, draft: true }, context)).toMatchObject({
        ok: true,
  ...
  test("agent publication and draft discovery report invalid source without changing publication", async () => {
    ...
    for (const content of [
      ...
      JSON.stringify({
        actions: [{ name: "convert", title: "Convert", description: "Convert", entry: "missing.ts", inputSchema: {}, outputSchema: {} }],
      }),
    ]) {
      const resource = await artifacts.create(
        { title: "Invalid manifest", source: { entry: "main.ts", files: [...source.files, { path: "app.actions.json", content }] } },
        owner,
      );
      try {
        expect(await artifactCodeHandlers.code_actions({ id: resource.id, draft: true }, context)).toMatchObject({
          ok: false,
          error: { code: "COMPILE_FAILED", status: 400, message: expect.stringContaining("app.actions.json") },
        });
        expect(await artifactCodeHandlers.code_publish({ id: resource.id, expectedRevision: 1, note: "Test" }, context)).toMatchObject({
          ok: false,
          error: { code: "COMPILE_FAILED", status: 400 },
        });
  ...
  test("shared binary files have independent atomic byte budgets, pagination and no count ceiling", async () => {
    ...
    const settings = spyOn(app.settings, "get").mockImplementation(
      async (key) =>
        ({
          "assistant.storage_total_mib": total,
          "assistant.storage_file_mib": single,
          "assistant.rsql_url": "",
    ...
    try {
      ...
      const write = (key: string, data: Uint8Array) =>
        artifacts.storage(resource.id, { area: "files", operation: "write", key }, reader, false, data);
      ...
      await expect(artifacts.storage(resource.id, { area: "files", operation: "read", key: "binary" }, stranger)).rejects.toMatchObject({
        code: "ACCESS_DENIED",
      });
      await expect(artifacts.storage(resource.id, { area: "files", operation: "read", key: "binary" }, reader, true)).rejects.toMatchObject(
        { code: "ACCESS_DENIED" },
      );
      ...
      expect((await api.request(endpoint, { method: "PUT", headers: { "content-type": "application/pdf" }, body: payload })).status).toBe(
        200,
      );
      ...
      expect(
        (
          await api.request(`/${resource.id}/storage`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ area: "files", operation: "read", key: "http-file" }),
          })
        ).status,
      ).toBe(400);
      ...
      await artifacts.storage(
        resource.id,
        { area: "kv", operation: "write", key: "state", content: JSON.stringify("x".repeat(600000)) },
        reader,
      );
      total = 2;
      single = 2;
      ...
      if (cleanup) await artifacts.remove(cleanup, owner);
  ...
  test("artifact revisions store JSON objects and retain immutable source", async () => {
    ...
    const rows = await sql<
      { revision: number; kind: string; source: string }[]
    >`SELECT revision,jsonb_typeof(source) AS kind,source::text AS source
  ...
  test("personal HTTP secrets stay encrypted, scoped and bound; requests execute only once", async () => {
    ...
    await artifacts.grant(resource.id, { type: "user", userId: reader.user.id }, "read", owner);
    const saved = await httpService.save(
      scope,
      {
        name: "crm",
    ...
    const [stored] = await sql<
      { encrypted: string }[]
    >`SELECT encrypted FROM assistant.http_secrets WHERE user_id=${owner.user.id}::uuid AND scope=${"resource:" + resource.id}`;
    ...
    await expect(
      httpService.prepare({ ...call, request: { ...call.request, url: "https://api.example.com/different" } }, owner),
    ).rejects.toMatchObject({ code: "HTTP_UNKNOWN" });
    let sent = 0;
    const send: Parameters<typeof httpService.execute>[4] = async (request) => {
      sent++;
      expect(request.headers.authorization).toBe("Bearer fixture-key-not-public");
    ...
    const results = await Promise.allSettled([
      httpService.execute(call.id, true, owner, new AbortController().signal, send),
      httpService.execute(call.id, true, owner, new AbortController().signal, send),
    ]);
    expect(results.filter((result) => result.status === "fulfilled")).toHaveLength(1);
    expect(sent).toBe(1);
    await expect(httpService.execute(call.id, true, owner, new AbortController().signal, send)).rejects.toMatchObject({
      code: "HTTP_UNKNOWN",
    });
    const uncertain = make();
    await httpService.prepare(uncertain, owner);
    await expect(
      httpService.execute(uncertain.id, true, owner, new AbortController().signal, async () => {
        sent++;
        throw new Error("fixture-key-not-public");
      }),
    ).rejects.toMatchObject({ message: "HTTP_UNKNOWN" });
    await expect(httpService.execute(uncertain.id, true, owner, new AbortController().signal, send)).rejects.toMatchObject({
      code: "HTTP_UNKNOWN",
    });
    expect(sent).toBe(2);
    const denied = make();
    await httpService.prepare(denied, owner);
    await expect(httpService.execute(denied.id, false, owner, new AbortController().signal, send)).rejects.toMatchObject({
      code: "HTTP_DENIED",
    });
    expect(sent).toBe(2);
    const changed = make();
    await httpService.prepare(changed, owner);
    const replacement = await httpService.save(scope, { ...saved, value: "replacement", expectedRevision: saved.revision }, owner);
    await expect(httpService.execute(changed.id, true, owner, new AbortController().signal, send)).rejects.toMatchObject({
      code: "HTTP_CONFLICT",
    });
    ...
    await artifacts.remove(resource.id, owner);
    await artifacts.remove(other.id, owner);
  });
  ...
  test("descriptions round-trip without source writes erasing them", async () => {
    ...
    const updated = await artifacts.update(
      app.id,
      { title: app.title, description: "Updated description.", expectedRevision: saved.revision, source },
      owner,
    );
  ...
  test("only admins see unpublished drafts; users can use a published version", async () => {
    ...
    await expect(artifacts.update(id, { title: "Changed", source, expectedRevision: 1 }, reader)).rejects.toMatchObject({
      code: "ACCESS_DENIED",
    });
  ...
  test("reviewed app grants reject stale approval and serialize concurrent changes", async () => {
    ...
    const input = {
      ...
      principal: { type: "user" as const, userId: reader.user.id },
    ...
    expect(await artifactCodeHandlers.code_access_change(input, { ...context, review: true })).toMatchObject({
      ok: false,
      error: { code: "CONFLICT" },
    });
    ...
    expect(
      await artifactCodeHandlers.code_access_change(
        { id: resource.id, accessId: manager.id, expectedAccessRevision: accessRevision(latest), permission: null },
        context,
      ),
  ...
  test("generic App file copies preserve binary bytes with Use and reject stale overwrite or denial", async () => {
    ...
    await artifacts.storage(
      from.id,
      { area: "files", operation: "write", key: "invoice.pdf", mediaType: "application/pdf" },
      owner,
      false,
      bytes,
    );
    ...
    expect(await artifactCodeHandlers.code_file_copy(input, { ...context, review: true })).toMatchObject({
      ok: true,
      data: { data: { message: expect.stringContaining("other authorized users") } },
    });
    ...
    await expect(studioFiles.copy(file!.reference, destination, null, stranger, context.signal)).rejects.toMatchObject({
      code: "ACCESS_DENIED",
    });
    ...
    await expect(
      studioFiles.copy(current!.reference, { ...destination, path: "cancelled.pdf" }, null, reader, abort),
  ...
  test("storage reviews reject intervening writes and file versions survive delete/recreate", async () => {
    ...
    expect(
      await artifactCodeHandlers.code_storage_delete({ ...input, expectedStorageRevision: current.storageRevision }, context),
    ...
    await expect(
      artifacts.storage(resource.id, { area: "kv", operation: "delete", key: "state" }, owner, true, undefined, {
  ...
  test("agent file writes persist incomplete code, preserve helpers and enforce permissions", async () => {
    ...
    expect(await write("helper.ts", "export const value = 42;")).toMatchObject({
      ok: true,
    ...
    expect(await artifactCodeHandlers.code_read({ id, path: "main.ts", offset: 0 }, context)).toMatchObject({
      ok: true,
      data: { data: { content: "export default !!!", complete: true } },
    });
    expect(
      await artifactCodeHandlers.code_write(
        { id, expectedRevision: 1, files: [{ path: "main.ts", content: "x" }] },
        { ...context, ...stranger },
      ),
    ).toMatchObject({ ok: false, error: { code: "ACCESS_DENIED" } });
    expect(await artifactCodeHandlers.code_remove({ id, path: "main.ts" }, context)).toMatchObject({
      ok: true,
  ...
  test("two browsers cannot execute one tool call twice and interrupted claims never replay", async () => {
    ...
    try {
      ...
      await expect(clientCalls.claim({ ...first, input: { ...input, id: "Xyz789" } }, owner)).rejects.toMatchObject({
        code: "INVALID_INPUT",
      });
  ...
  test("publication versions preserve metadata, rollback atomically publishes, and sharing is independent", async () => {
    ...
    expect(restored.title).toBe("Budget");
    expect(restored.icon).toBe("ti ti-wallet");
  ...
  test("script capabilities require real approval, remember scopes and never execute twice", async () => {
    ...
    try {
      ...
      await expect(runtimeCapabilities.resolve(denied.id, { approved: false }, stranger, caller)).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
      ...
      await runtimeCapabilities.prepare(changed, owner, caller);
      ...
      expect(await runtimeCapabilities.resolve(approved.id, { approved: true }, owner, caller)).toEqual({
        status: "completed",
        result: { ok: true, data: { data: { written: true } } },
      });
      const [stored] = await sql<
        { request: string; prepared: string; result: string }[]
      >`SELECT jsonb_typeof(request) AS request,jsonb_typeof(prepared) AS prepared,jsonb_typeof(result) AS result
  ...
  test("binary streams require this user's completed consent and fresh resource access", async () => {
      ...
      defineCapabilities({
        ...
        queries: {
          read: {
            ...
            data: z.object({}),
            openWorld: false,
            stream: { direction: "read", maxBytes: 10, read: async () => new Response("pdf") },
            run: () => ok({ data: {}, stream: descriptor }),
          },
        },
      }),
    );
    const catalog = spyOn(capabilityClient, "getCapabilityCatalogApp").mockResolvedValue({
      ok: true,
      data: { appId: "binary", appName: "Binary", appDescription: "", appIcon: "ti ti-file", manifest },
    });
  ...
  test("shared resources require consent for queries and ignore personal remembered actions", async () => {
    ...
    const definitions = defineCapabilities({
      ...
      actions: {
        write: {
          ...
          destructive: false,
          openWorld: false,
          review: async () => ok({ message: "Write?", approvalScope: "private" }),
          run: async () => ok({ data: {} }),
        },
      },
    });
    ...
    const catalog = spyOn(capabilityClient, "getCapabilityCatalogApp").mockResolvedValue({
      ok: true,
      data: { appId: "consent", appName: "Consent", appDescription: "", appIcon: "ti ti-app-window", manifest },
    });
    const review = spyOn(capabilityClient, "reviewCapabilityAction").mockResolvedValue({
      ok: true,
    ...
    try {
      ...
      expect(await runtimeCapabilities.prepare(read, reader, {})).toMatchObject({
        status: "approval",
        allowAlways: false,
        resource: { title: "Shared data app" },
      });
      expect(execute).not.toHaveBeenCalled();
      await expect(runtimeCapabilities.resolve(read.id, { approved: true, remember: "always" }, reader, {})).rejects.toMatchObject({
        code: "INVALID_INPUT",
      });
  ...
  test("reviewed App deletion rejects storage changes and removes only the reviewed resource", async () => {
    ...
    expect(await artifactCodeHandlers.code_delete(input, { ...context, review: true })).toMatchObject({
      ok: true,
      data: { data: { message: expect.stringContaining("Delete review") } },
    });
    ...
    expect(
      await artifactCodeHandlers.code_delete({ ...input, expectedManagementRevision: latest.managementRevision }, context),
  ...
  test("resource managers can delete shared data and queue database cleanup; use access cannot", async () => {
    ...
    expect(
      await sql`SELECT 1 FROM assistant.artifact_storage WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${resource.id})`,
    ).toHaveLength(0);
    expect(
      await sql`SELECT 1 FROM assistant.artifact_publications WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${resource.id})`,
    ).toHaveLength(0);
  ...
  test("databases are lazy, reconnect idempotently and reject writes through SQL", async () => {
    ...
    const settings = spyOn(app.settings, "get").mockImplementation(
      async (key) =>
        ({
          ...
          "assistant.rsql_url": url,
          "assistant.rsql_api_token": token,
        })[key],
    );
    ...
    try {
      expect(
        await sql`SELECT 1 FROM assistant.artifact_databases WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${applet.id})`,
      ).toHaveLength(0);
      ...
      expect(creates).toBe(1);
      expect(await artifactCodeHandlers.code_sql({ id: applet.id, sql: "DELETE FROM todos", params: [] }, context)).toMatchObject({
        ok: false,
        error: { code: "DB_SQL_UNSUPPORTED" },
      });
      expect(
      ...
      expect(
        await sql`SELECT 1 FROM assistant.artifact_databases WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${fork.id})`,
      ).toHaveLength(0);
    } finally {
      settings.mockRestore();
  ...
  test.todo("real rsql imports and rejoins 2500 rows without duplicating retries", async () => {
    ...
    try {
      ...
      await call({
        ...
        name: "ledger_rows",
        columns: [
          { name: "import_key", type: "text", unique: true, not_null: true },
      ...
      const rows = Array.from({ length: 2500 }, (_, i) => ({
        ...
        payload: JSON.stringify([i, "12.34"]),
      }));
      ...
      expect(await call({ operation: "query", sql: "SELECT count(*) AS total FROM ledger_rows", params: [] })).toMatchObject({
        data: [{ total: 2500 }],
      });
      expect(
        await artifactCodeHandlers.code_sql(
  ...
  test.todo("database clear preserves schema and every attempted write invalidates reviewed state", async () => {
    ...
    try {
      ...
      expect(await artifactDatabase.clear(resource.id, afterFailure.generation!, afterFailure.dataRevision!, owner)).toEqual({
        completed: true,
  ...
  test.todo("partial clear reports progress and releases serialization after cancellation", async () => {
    ...
    const proxy = Bun.serve({
      ...
      async fetch(request) {
        ...
        const base = new URL(requireInfraUrl("rsql"));
    ...
    const settings = spyOn(app.settings, "get").mockImplementation(
      async (key) =>
        ({
          "assistant.storage_total_mib": 250,
          "assistant.storage_file_mib": 50,
          "assistant.rsql_url": proxy.url.toString(),
          "assistant.rsql_api_token": "artifact-test-only",
        })[key],
    );
    ...
    const settings = spyOn(app.settings, "get").mockImplementation(
      async (key) =>
        ({
          "assistant.storage_total_mib": 250,
          "assistant.storage_file_mib": 50,
          "assistant.rsql_url": upstream.url.origin,
          "assistant.rsql_api_token": "private-fixture-token",
        })[key],
    );
    try {
  ...
  test("Studio management preserves runtime access and scopes shared clears", async () => {
    ...
    const [grant] = await sql<
      { id: string }[]
    >`INSERT INTO auth.access(user_id,permission) VALUES(${reader.user.id}::uuid,'read') RETURNING id`;
    ...
    await expect(artifacts.storage(resource.id, { area: "kv", operation: "list" }, reader, true)).rejects.toMatchObject({
      code: "ACCESS_DENIED",
    });
    ...
    for (const operation of [{ operation: "tables.list" }, { operation: "rows.insert", table: "ledger", rows: [{ amount: 1 }] }])
      await expect(artifactDatabase.call(resource.id, operation, reader, undefined, "maintenance")).rejects.toMatchObject({
        code: "ACCESS_DENIED",
      });
    ...
    await expect(artifactDatabase.call(resource.id, { operation: "tables.list" }, reader, undefined, "inspect")).rejects.toMatchObject({
      code: "ACCESS_DENIED",
    });
    ...
    expect(await artifacts.storage(resource.id, { area: "files", operation: "read", key: "keep.txt" }, reader)).toMatchObject({
      item: { data: new Uint8Array([97]) },
    });
  ...
  test.todo("Studio database backup and reset preserve source and rotate namespace generations", async () => {
    ...
    try {
      ...
      expect(
        await sql`SELECT 1 FROM assistant.artifact_databases WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${resource.id})`,
      ).toHaveLength(0);
      ...
      await artifactDatabase.call(
        resource.id,
        { operation: "rows.insert", table: "ledger", rows: [{ amount: 1200 }] },
        owner,
        undefined,
        "maintenance",
      );
      ...
      try {
        ...
        const before =
          await sql`SELECT bytes,version FROM ai.files WHERE conversation_id=${conversation.id}::uuid AND path='/database.sqlite'`;
        expect(await artifactCodeHandlers.code_database_export({ id: resource.id, path: "/database.sqlite" }, context)).toMatchObject({
          ok: false,
          error: { code: "CONFLICT", status: 409, message: expect.stringContaining("nothing was written") },
        });
        expect(
          await sql`SELECT bytes,version FROM ai.files WHERE conversation_id=${conversation.id}::uuid AND path='/database.sqlite'`,
        ).toEqual(before);
      } finally {
        conversationRead.mockRestore();
        await sql`DELETE FROM ai.files WHERE conversation_id=${conversation.id}::uuid`;
      ...
      await Promise.all([
        artifactDatabase.reset(resource.id, oldGeneration, owner),
        artifactDatabase.reset(resource.id, oldGeneration, owner),
      ]);
  ...
  test("database settings redact credentials and reject changing an in-use server", async () => {
    ...
    const settings = spyOn(app.settings, "get").mockImplementation(
      async (key) =>
        ({
          ...
          "assistant.storage_file_mib": 50,
          "assistant.rsql_url": upstream.url.origin,
          "assistant.rsql_api_token": "secret-fixture",
        })[key],
    );
    ...
    try {
      ...
      await expect(artifactDatabase.configure({ url: "http://different.invalid", token: "new" }, administrator)).rejects.toMatchObject({
  ...
  test("platform administration is explicit and deletion queues database cleanup", async () => {
    ...
    expect(
      await sql`SELECT 1 FROM assistant.artifact_storage WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${applet.id})`,
    ).toHaveLength(0);
    expect(
      await sql`SELECT 1 FROM assistant.artifact_databases WHERE artifact_id=(SELECT id FROM assistant.artifacts WHERE short_id=${applet.id})`,
    ).toHaveLength(0);
  ...
  test("shared storage survives publications but is not copied into forks", async () => {
    ...
    await artifacts.publish(app.id, 1, owner, "Initial release");
    await artifacts.grant(app.id, { type: "user", userId: reader.user.id }, "read", owner);
    expect(await artifacts.storage(app.id, { area: "kv", operation: "read", key: "counter" }, reader)).toMatchObject({
      item: { content: "42" },
    });
    await artifacts.storage(app.id, { ...write, content: "43" }, reader);
    await artifacts.storage(
      app.id,
      { area: "files", operation: "write", key: "input.csv", mediaType: "text/csv" },
      reader,
      false,
    ...
    await expect(artifacts.storage(app.id, { area: "kv", operation: "read", key: "counter" }, reader)).rejects.toMatchObject({
      code: "NOT_FOUND",
    });
  ...
  test("project app selection only exposes manageable candidates and linking requires both admin grants", async () => {
    ...
    const project = spyOn(aiProjects, "get").mockImplementation(async (id, subject, permission) => {
      ...
      return id === projectId && (projectAdmin || (projectReader && permission === "read"))
        ? {
            ...
            permission: projectAdmin ? "admin" : "read",
            revision: 1,
            createdAt: "",
            updatedAt: "",
          }
    ...
    try {
      ...
      await artifacts.grant(app.id, { type: "user", userId: reader.user.id }, "read", owner);
      expect((await artifacts.projectApps("linkproj", owner, 1, "Project selection", true)).items).toMatchObject([
        { id: app.id, canManage: true, linked: false, published: false },
      ]);
  ...
  test("project membership grants published Use everywhere and revocation never changes direct grants", async () => {
    ...
    try {
      ...
      expect(await artifacts.storage(script.id, { area: "kv", operation: "read", key: "shared" }, stranger)).toMatchObject({
        item: { content: "42" },
      });
      await expect(artifacts.writeFile(script.id, "main.js", "export default () => 2", stranger)).rejects.toMatchObject({
        code: "ACCESS_DENIED",
      });
      await expect(artifacts.linkProject(script.id, projectId, false, stranger)).rejects.toMatchObject({ code: "ACCESS_DENIED" });
      await expect(artifacts.grant(script.id, { type: "authenticated" }, "read", stranger)).rejects.toMatchObject({
        code: "ACCESS_DENIED",
      });
      const successorGrant = await artifacts.grant(script.id, { type: "user", userId: reader.user.id }, "admin", owner);
      const creatorGrant = (await artifacts.access(script.id, owner)).find(
        (entry) => entry.principal.type === "user" && entry.principal.userId === owner.user.id,
      )!;
      ...
      await expect(artifacts.storage(script.id, { area: "kv", operation: "read", key: "shared" }, stranger)).rejects.toMatchObject({
        code: "ACCESS_DENIED",
      });
      ...
      await artifacts.remove(script.id, owner);
    }
  });
  ...
  test("edit creates an unsent chat draft and indexes the app reference for admins only", async () => {
    ...
    const create = spyOn(aiConversations, "createConversation").mockImplementation(async (input) => ({
      ...
      unreadCompletion: false,
      projectId: null,
      draft: { content: input.draft ?? [], revision: 1, updatedAt: null },
    ...
    try {
      ...
      expect(create.mock.calls[0]![0].draft).toEqual([
  ...
  test("chat presentations retain input bytes, deduplicate and enforce chat ownership", async () => {
    ...
    await sql`INSERT INTO ai.conversations(id,created_by_user_id) VALUES(${conversationId}::uuid,${owner.user.id}::uuid)`;
    const conversation = spyOn(aiConversations, "getConversation").mockImplementation(async (request) =>
      request.ownerUserId !== owner.user.id || request.conversationId !== conversationId
        ? null
        : {
            id: conversationId,
            shortId: "abc234",
  ...
  test.todo("server-owned code survives caller detachment, deduplicates calls and never replays a lost host", async () => {
    ...
    try {
      ...
      const exampleSettings = spyOn(app.settings, "get").mockImplementation(
        async (key) =>
          ({
            "assistant.storage_total_mib": 250,
            "assistant.storage_file_mib": 50,
      ...
      try {
        for (const [folder, action, input, expected] of [
          ...
          [
            "invoice-matcher",
            "linkTransaction",
            { transactionKey: "bank-42", invoicePath: "invoices/INV-7.pdf" },
            { linked: true, unchanged: false },
          ],
        ] as const) {
          ...
          try {
            ...
            if (folder === "invoice-matcher")
              await artifacts.storage(
                example.id,
                { area: "files", operation: "write", key: "invoices/INV-7.pdf", mediaType: "application/pdf" },
                owner,
                false,
                new TextEncoder().encode("Fixture invoice"),
              );
            await artifacts.publish(example.id, 1, owner, "Example release");
            const invoke = {
              ...call,
              name: "code_action" as const,
              callId: `${folder}-action`,
              args: { id: example.id, action, publishedVersion: 1, input },
            };
            expect(await wait(invoke)).toMatchObject({ status: "done", result: { status: "ready", output: JSON.stringify(expected) } });
            if (folder === "importer")
              expect(await wait({ ...invoke, callId: "importer-second-session" })).toMatchObject({
                status: "done",
                result: { output: '{"inserted":0,"skipped":1}' },
              });
            if (folder === "invoice-matcher")
              expect(await wait({ ...invoke, callId: "invoice-link-repeat" })).toMatchObject({
                status: "done",
      ...
      const exported = await wait({
        ...
        name: "code_export",
        callId: "export-data",
        args: { runId: produce.callId, name: "data.json" },
      });
      ...
      const written = await artifactCodeHandlers.code_write(
        {
          id: resource.id,
          expectedRevision: 1,
          entry: "main.ts",
          files: [
            { path: "data.json", fromFile: fileReference },
            {
              path: "main.ts",
              content: 'import data from "./data.json"; export default()=>({rows:data.length,first:data[0],last:data.at(-1)});',
            },
          ],
        },
        context,
      ...
      expect((await artifacts.get(resource.id, owner)).source.files.find((file) => file.path === "data.json")?.content).toBe(
        JSON.stringify(generated),
      );
      expect(await wait({ ...call, callId: "run-import", args: { id: resource.id } })).toMatchObject({
        status: "done",
        result: {
          status: "ready",
          id: resource.id,
          revision: 2,
          output: JSON.stringify({ rows: 1000, first: generated[0], last: generated.at(-1) }),
        },
      });
      const stale = await artifactCodeHandlers.code_write(
        {
          id: resource.id,
          expectedRevision: 2,
          files: [{ path: "data.json", fromFile: { scope: "chat", id: context.conversationId, path: file.path, version: "2" } }],
        },
        context,
      );
      ...
      try {
        ...
        for (let i = 0; i < 200 && !approval; i++) {
          const state = await agentHost.call(network, context);
          approval = state.approvals[0];
          if (!approval) await Bun.sleep(25);
        }
