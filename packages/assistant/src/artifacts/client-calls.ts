import { aiConversations, aiToolAudit, CODE_RUNTIME_TOOL_NAMES, CodeRuntimeInput, parseCodeToolInput } from "@k2b/cloud/ai";
import { userFromActor } from "@k2b/cloud/server";
import { sql } from "bun";
import { z } from "zod";
import { ArtifactError, type ArtifactIdentity } from "./service";

export const ClientCall = z
  .object({
...
const Output = z
  .json()
  .refine((value) => new TextEncoder().encode(JSON.stringify(value)).byteLength <= 256 * 1024, "Tool result exceeds 256 KiB");
...
async function authorize(call: z.infer<typeof ClientCall>, identity: ArtifactIdentity) {
  ...
  if (!user || identity.accessSubject.type !== "user" || identity.accessSubject.userId !== user.id)
  ...
  if (
    !active ||
    (active.turn.id !== call.turnId && active.turn.shortId !== call.turnId) ||
    active.turn.status !== "waiting_for_action" ||
    !block ||
    block.kind !== "tool" ||
    !CODE_RUNTIME_TOOL_NAMES.some((name) => name === block.name) ||
    block.status !== "awaiting_client"
  )
    throw new ArtifactError("CONFLICT");
  // Schema parsing gives canonical property order and strips no unknown fields.
  if (JSON.stringify(CodeRuntimeInput.parse(parseCodeToolInput(block.name, block.args))) !== JSON.stringify(call.input))
...
export const clientCalls = {
  async claim(input: unknown, identity: ArtifactIdentity) {
    const call = ClientCall.parse(input),
      { user, turnId, conversationId, toolName } = await authorize(call, identity);
    const result = await sql.begin(async (db) => {
      ...
      const [row] = await db<
        { result: unknown; completed: boolean; expired: boolean }[]
      >`SELECT result, result IS NOT NULL AS completed, heartbeat_at < now() - interval '2 minutes' AS expired
  ...
  async complete(input: unknown, identity: ArtifactIdentity) {
    const call = ClientCallResult.parse(input),
      { user, turnId } = await authorize(call, identity);
