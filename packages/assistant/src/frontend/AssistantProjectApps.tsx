...
export function AssistantProjectApps(props: { project: AiProject; initialPage: AppPage }) {
  ...
  const page = (value: AppPage) => ({
    ...value,
    items: value.items.map((app) => ({
      ...app,
  ...
  return (
    <AssistantProjectLinks
      ...
      title={text("Studio Apps")}
      addLabel={text("Link a Studio App")}
      searchLabel={text("Search Studio Apps…")}
      notice={text("Members can use linked, published apps and their shared app data. Editing permissions stay unchanged.")}
      emptyLabel={text("No linked apps yet.")}
      noResultsLabel={text("No matching apps that you manage.")}
      load={async (cursor, query, available, signal) =>
        page(await artifactClient.projectApps(props.project.id, cursor, query, available, signal))
      }
