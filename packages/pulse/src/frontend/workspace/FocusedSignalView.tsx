import { Button, DataTable, type DataTableColumn, TextInput } from "@k2b/ui";
import { type Accessor, createMemo, type JSX, type Setter, Show } from "solid-js";
...
import { type PulseDateContext, plural, stateRowId } from "./helpers";
...
export default function FocusedSignalView(props: FocusedSignalViewProps) {
  ...
  const rowsLabel = createMemo(() =>
    kind() === "event"
      ? t().eventCount({ count: props.events().length })
      : t().variantCount({ count: kind() === "metric" ? props.metricSeries().length : props.states().length }),
  );
