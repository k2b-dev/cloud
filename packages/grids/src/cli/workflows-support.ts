...
export const WORKFLOW_REFERENCE = {
  language: {
    ...
    atomicPredicates:
      "atomicRecords.checks accepts stored-field table/where checks or query: {source, parameters} GQL checks with formulas, grouping, aggregates and having. Each check uses assert: empty|notEmpty and an optional message. Locks accept Record and record-list references (at most 100 distinct Records); include the shared coordination Record for concurrent runs. Ordered changes support createRecord, updateRecord, deleteRecord and finalizeRecord. deleteRecord takes record and optional audit answers; it locks its target and moves a mutable record to trash in the same transaction. createRecord accepts optional finalize: true to create and finalize in one transaction, with enabled direct Finalization; a failure leaves no draft. Optional validateDocuments: [{template, record}] validates profile inputs after changes and rolls back all changes on failure. For finalized oncePerFinalizedRecord targets it also reserves the document number and freezes template, snapshot and render context; generate the PDF in a later step. Subsequent generation reuses that reservation.",
    recordLifecycle:
