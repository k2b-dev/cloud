...
export const WORKFLOW_REFERENCE = {
  language: {
    ...
    fileCapture:
      "parseDocument requires record: inputs.<record>, a bound File field, and format: camt.052.001.08. Exactly one UTF-8 XML attachment is captured with its original bytes and typed account reports under the cumulative 5 MiB run capture budget. Optional saveAs stores a reference; public step results expose {kind:fileSnapshot,stepKey,sha256,rowCount,capturedAt}, never its internal capture UUID. rowCount counts reports, not transactions. Pass the saved variable as generateDocument.data. No payment matching or record writes; missing transaction amounts, direction, status, reversal and bank pagination stay explicit. Inspect with workflow-runs file <run-short-id> <step-key> --sha256 <capture-hash> [--json], or download-file with --out. Dry run does not parse or capture the file. See Help grids-camt for the full contract.",
    conditions: {
