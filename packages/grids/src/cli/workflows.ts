...
export const workflowRunCommands = [
  ...
  command("workflow-runs download-documents", {
    summary: "Download all documents of a workflow run: one PDF if all are PDFs, otherwise one ZIP",
    args: { run: arg.required({ description: "Workflow run public id" }) },
    flags: { out: flag.string({ description: "Output path (.pdf when every document is a PDF, otherwise .zip)" }) },
    async run({ ctx, args, flags }) {
      await writeApiFile(
        ctx,
