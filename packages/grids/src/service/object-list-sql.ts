...
const normalizedCalculation = (column: ObjectListColumn, expression: FormulaSqlExpression): FormulaSqlCompileResult => {
  ...
  if (type === "numeric") {
    ...
    // PostgreSQL and JavaScript regex dialects differ. Never silently change a constraint.
...
export const compileObjectListRow = (
  ...
  for (const column of config.fields) {
    ...
    columns.set(column.id, { sql: projection ?? sql`${sql.unsafe(rowAlias)}.data->${column.id}`, type: scalarSqlTypeForField(column) });
  }
...
export const compileObjectListValue = (
  field: { id: string; config: unknown },
  recordAlias: string,
  ...
  const rows = sql`jsonb_array_elements(${source}) WITH ORDINALITY AS ${sql.unsafe(rowAlias)}(data, position)`;
  const errorSql = sql`CASE WHEN ${stored} OR ${absent} THEN false ELSE (SELECT COALESCE(bool_or(${row.plan.errorSql}), false) FROM ${rows} ${row.plan.joins}) END`;
