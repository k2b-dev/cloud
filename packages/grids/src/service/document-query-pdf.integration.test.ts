...
const renderTest = process.env.GRIDS_PDF_TEST === "1" ? test : test.skip;
renderTest("real Gotenberg renders multiple records, long nested items, repeated headers and an empty selection", async () => {
  const directory = process.env.GRIDS_PDF_REVIEW_DIR ?? await mkdtemp(join(tmpdir(), "grids-query-pdf-"));
  try {
    for (const empty of [false, true]) {
      const rows = empty ? [] : Array.from({ length: 3 }, (_, record) => ({
        title: `Expense ${record + 1} - Müller & Partner <review>`,
        items: Array.from({ length: 45 }, (_, item) => ({ description: `Position ${record + 1}.${item + 1} - Reisekosten`, amount: "12.30" })),
      }));
      const result = await renderDocumentHtmlPdf({
        filename: "expenses.pdf", data: { rows, columns: [], document: { number: "REPORT-1" } },
        content: {
          ...
          footer: '<div style="font: 9px sans-serif; text-align: center; width: 100%">REPORT-1 - <span class="pageNumber"></span></div>',
        },
      }, "en", { config: { url: process.env.GRIDS_PDF_URL ?? "http://localhost:3001", timeoutMs: 30_000, maxHtmlBytes: 1_000_000, maxPdfBytes: 10_000_000 } });
      if (!result.ok) throw result.error;
      const path = join(directory, empty ? "empty.pdf" : "multi-record.pdf");
      await Bun.write(path, result.data.pdf);
      const extraction = Bun.spawn([Bun.env.PDFTOTEXT ?? "pdftotext", "-layout", path, "-"], { stdout: "pipe", stderr: "pipe" });
