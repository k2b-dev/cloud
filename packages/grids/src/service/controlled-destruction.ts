...
const destructionJob = job<{ runId: string }, void>({
  id: "grids:controlled-destruction",
  defaults: { leaseMs: 30_000, keyTtlMs: 24 * 60 * 60_000 },
  process: async ({ ctx }) => processRun(ctx.input.runId, () => ctx.heartbeat({ leaseMs: 30_000 })),
  after: async ({ ctx }) => {
    if (!ctx.error) return;
...
export const stopControlledDestructionJobs = (): void => destructionJob.stop();
