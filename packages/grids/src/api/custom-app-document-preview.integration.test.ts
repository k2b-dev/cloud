...
postgresTest(
  "published draft preview is an explicit pinned data-product grant without issuance",
  async () => {
    ...
    const render = spyOn(gridsService.document, "renderPdfPreview").mockImplementation(async (_template, data) => {
      ...
      return {
        ...
        footerHtml: null,
        pageCss: null,
        pdf: { pdf: new TextEncoder().encode("%PDF-preview"), contentType: "application/pdf" },
      };
    });
    try {
        ...
        pages: [
          {
            ...
            rows: [
              {
                id: "main",
                columns: [{ id: "content", span: 12, blocks: [{ id: "welcome", type: "markdown", markdown: "Draft portal" }] }],
              },
            ],
          },
          {
            ...
            parameters: { item: { type: "record", tableId: table, required: true } },
            record: { tableId: table, id: { source: "PARAMS", path: "item" } },
            rows: [
              {
                id: "main",
                columns: [{ id: "content", span: 12, blocks: [{ id: "record", type: "record", fieldIds: [field], documents }] }],
              },
            ],
          },
        ],
