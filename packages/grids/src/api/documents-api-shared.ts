...
import { documentCursorMatchesSort, loadDocumentDataSnapshots, loadDocumentWorkflowOrigins } from "../service/document-browse";
...
export const projectDocuments = async (documents: InternalDocumentSummary[]) => {
  const [templates, bases, tables, records, dataSnapshots, sourceCounts, workflowOrigins] = await Promise.all([
    projectPublicIds(
      "documentTemplate",
    ...
    projectPublicIds(
      "table",
      documents.flatMap((document) => (document.tableId ? [document.tableId] : [])),
    ),
    projectPublicIds(
      "record",
    ...
    loadDocumentWorkflowOrigins(documents.flatMap((document) => (document.workflowRunId ? [document.workflowRunId] : []))),
  ]);
  return documents.map(({ id, shortId, templateId, workflowRunId, snapshotId: _snapshotId, baseId, tableId, recordId, ...document }) => {
    const origin = workflowRunId === null ? null : workflowOrigins.get(workflowRunId);
    if (origin === undefined) throw new Error("Grids could not project a public workflow run id.");
    return {
      id: shortId,
      baseId: requiredPublicId(bases, baseId, "base"),
      tableId: tableId === null ? null : requiredPublicId(tables, tableId, "table"),
      recordId: recordId === null ? null : requiredPublicId(records, recordId, "record"),
      templateId: templateId === null ? null : requiredPublicId(templates, templateId, "document template"),
      workflowId: origin?.workflowId ?? null,
      workflowRunId: origin?.runId ?? null,
...
export const projectDocumentLinks = async (links: InternalDocumentLink[]) => {
  const [publicLinks, documents, bases, tables, records] = await Promise.all([
    ...
    projectPublicIds(
      "table",
      links.flatMap((link) => (link.tableId ? [link.tableId] : [])),
    ),
    projectPublicIds(
      "record",
      links.flatMap((link) => (link.recordId ? [link.recordId] : [])),
    ),
  ]);
...
const DocumentSourceFolderSchema = z.union([ShortIdSchema, z.string().regex(/^workflow:[A-Za-z0-9]{6}$/)]);

const DocumentCatalogQueryShape = {
  workflow: ShortIdSchema.optional().describe("Workflow public ID: Documents generated by any of its runs."),
  template: ShortIdSchema.optional().describe("Document template public ID."),
  table: ShortIdSchema.optional().describe("Table public ID: Documents directly bound to one of its records."),
  mediaType: DocumentMediaTypeSchema.optional().describe("Media type of the primary artifact, for example application/zip."),
  sort: z.enum(DOCUMENT_CATALOG_SORTS).optional().default("newest"),
};

const cursorMatchesSort = (query: { cursor: string; sort: (typeof DOCUMENT_CATALOG_SORTS)[number] }) =>
  documentCursorMatchesSort(query.cursor, query.sort);
const cursorSortMismatch = { message: "Document cursor belongs to another sort order", path: ["cursor"] };

/** Filters, sort and page of the Base-wide catalog as a flat list. */
export const BaseDocumentListQuerySchema = PublicDocumentPageQuerySchema.extend({
  q: z.string().optional().default(""),
  ...DocumentCatalogQueryShape,
}).refine(cursorMatchesSort, cursorSortMismatch);

export const BaseDocumentBrowseQuerySchema = DocumentBrowseQuerySchema.omit({ tags: true })
  .extend({
    ...
    path: z
      ...
      .pipe(
        z.union([z.tuple([]), z.tuple([DocumentSourceFolderSchema]), z.tuple([DocumentSourceFolderSchema, z.string().regex(/^\d{4}$/)])]),
      ),
    ...DocumentCatalogQueryShape,
  })
  .refine(cursorMatchesSort, cursorSortMismatch);

/** Maps validated catalog query parameters to the service's filter shape. */
export const documentCatalogFilters = (query: { workflow?: string; template?: string; table?: string; mediaType?: string }) => ({
  workflowId: query.workflow ?? null,
