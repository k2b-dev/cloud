...
// Keep the live-socket module fixture inside its own process so package-wide
// test runs never replace the real `@k2b/cloud/browser/live` used by other suites.
if (process.env.GRIDS_LIVE_EVENTS_CHILD !== "1") {
  test("Grids live event adapters subscribe, acknowledge, and resynchronize", async () => {
    const child = Bun.spawn([process.execPath, "test", import.meta.path], {
      env: { ...process.env, GRIDS_LIVE_EVENTS_CHILD: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
