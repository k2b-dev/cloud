...
for (const issuancePolicy of ["repeatable", "oncePerFinalizedRecord"] as const) {
  domTest(`${issuancePolicy}: generation blocks dismissal while pending and preserves the key after an uncertain response`, async () => {
    ...
    const button = (label: string) =>
      Array.from(dom.document.querySelectorAll("button")).find((item) => item.textContent?.trim() === label)!;
    try {
      ...
      if (issuancePolicy === "oncePerFinalizedRecord")
        expect(dom.document.body.textContent).toContain("Finalize the record before issuing");
      if (issuancePolicy === "repeatable") {
        button("Render preview").click();
        await Bun.sleep(15);
      }
      button(issuancePolicy === "repeatable" ? "Generate Document" : "Issue or retrieve").click();
      await Bun.sleep(5);
