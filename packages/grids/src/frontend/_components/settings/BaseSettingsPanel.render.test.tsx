...
describe("Grids Base settings composition", () => {
  ...
  test("opens the server-filtered Tables overview in its own workspace dialog", () => {
    ...
    const messages = readFileSync(join(import.meta.dir, "messages.ts"), "utf8");
    ...
    expect(source).toContain("messages().reviewTables");
    expect(source).toContain("messages().historyAndFinalization");
    expect(messages).toContain('reviewTables: "Review Tables"');
    expect(messages).toContain('historyAndFinalization: "History & Finalization"');
  ...
  test("keeps scoped holds in their own lifecycle category with explicit consequences", () => {
    ...
    const panel = readFileSync(join(import.meta.dir, "BaseSettingsPanel.tsx"), "utf8");
    const messages = readFileSync(join(import.meta.dir, "messages.ts"), "utf8");
    expect(source).toContain("messages().activeHolds");
    expect(source).toContain("query.create");
    expect(source).toContain("messages().activeHoldsDescription");
    expect(source).toContain("messages().holdsGuidance");
    expect(messages).toContain('preservationHolds: "Preservation holds"');
    expect(messages).toContain('activeHoldsDescription: "Every active hold must be released separately."');
    expect(messages).toContain("A Table hold also prevents deleting its parent Base");
    ...
    expect(source).toContain("messages().searchTablesFailed");
    expect(source).toContain("messages().loadHoldsFailed");
    expect(source).toContain("setTableId(null)");
    expect(source).toContain("holds.refresh()");
    expect(source).toContain("messages().holdCreatedRefreshFailed");
    expect(source).toContain("messages().holdReleasedRefreshFailed");
