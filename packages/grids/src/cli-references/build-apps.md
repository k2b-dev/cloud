...
For draft PDF previews in a Custom App, opt in with `record.documents: {templateIds: [...], preview: true}`. This grants the template's queried data to app readers. Only saved, unfinalized records can be previewed; no document or number is issued. Source interpolation permits only `{{ record.id }}` and `{{ record.shortId }}`, without Liquid filters or tags. Republish after template or Base schema changes. See the installed Custom App API help for the complete contract.

Keep editing and issuance on separate pages: a workflow uses saved records, not unsaved form inputs. The billing template returns from its edit form to a read-only review page using `onSuccessNavigate`; preview and issuance live there.
...
For stock quantities, an append-only movement table can preserve receipts, issues and corrections. A computed balance is a read, not a reservation lock. Do not claim that checking a displayed SUM then writing a movement prevents overselling. Use an `atomicRecords` GQL check under a shared coordination lock to select budget violations, then assert `empty` and commit the movement in the same action. Include the proposed quantity in the check. Every competing path must use that lock; arbitrary direct writes are not protected by the workflow. Do not invent a SQL endpoint or workflow arithmetic function.
...
5. Validate GQL, formulas and workflow YAML with the installed compiler. Pass JSON/YAML through files. Read every created resource back. If a multi-command build stops, continue from the returned IDs; blindly replaying creates can duplicate resources. Form submission supports an explicit stable `idempotencyKey`; retry the exact body under that key. Unkeyed Form creation and record import can duplicate on retry.
...
Record targets use typed references such as `inputs.item`. Relation field values and relation-filter operands instead use public IDs, for example `${{ inputs.item.recordId }}`; internal UUIDs and labels are not authoring values. Atomic `table`/`where` checks support stored-field predicates; use `query: {source, parameters}` for formulas and grouped sums. Read changing facts inside that locked query, not from an earlier workflow parameter. See the [workflow reference](grids.md#workflows) for row-existence assertions and limits.
...
For `oncePerFinalizedRecord` templates, finalize once, then generate. A render failure leaves the Record finalized: retry generation with the same Record/template to complete or retrieve its original issuance, not to finalize again or reset state. `atomicRecords` checks App guards once after locks; generation checks again. Keep `availableWhen` eligible for generation after finalization. Put the initial-state assertion in an atomic check. Grants, publication and launcher validity still apply. Use `fail.message` or atomic `checks[].message` for safe recovery instructions; raw internal failures stay hidden from App readers.
...
Form relation pickers work with App access alone, exposing only record IDs and the published target's presentable labels. Changes to those fields or their formula dependencies require republishing; unrelated target fields do not.
...
- **Billing:** German EUR invoices, linked credit notes and commission self-billing, using ordinary Object lists, relations, Workflows and a Custom App. Positions belong directly to one bill, including self-billing; issuance freezes only that bill. There is no commission collection or automatic duplicate-obligation check. Generated documents are once-per-finalized-record. Drafts can be discarded. Payment forms save unconfirmed entries; a separate confirmation checks the finalized bill and freezes the payment before it affects the balance. This records actual transfers, not bank execution. Confirmed payments cannot be edited or moved to another bill. Review company, partner and bank data before use; the template is not accounting or legal certification.
