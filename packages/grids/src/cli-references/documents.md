...
Every Document shape (`document.list`, `document.read`, `document.create`, `documents get|list|browse|by-record` and `workflow-runs documents`) returns `downloadUrl` for the stored primary artifact and `artifacts[].downloadUrl` for each exact artifact, regardless of format. These are authenticated same-origin Cloud paths, not public shares. For bytes in Studio/code mode use `document.content.read` and the returned stream. Browser folder downloads collect primary files recursively into ZIP (up to 1,000 documents / 100 MiB); additional artifacts remain individual downloads. This live enumeration is not a backup snapshot.
