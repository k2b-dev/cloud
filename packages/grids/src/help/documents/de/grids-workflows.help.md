...
`validateDocuments: [{template, record}]` prüft Profileingaben nach Änderungen vor dem Commit; Fehler rollen auch Festschreibungen zurück. HTML scheitert mit `DOCUMENT_PROFILE_REQUIRED`. Bei festgeschriebenen Datensätzen mit `oncePerFinalizedRecord` reserviert der Schritt auch die Dokumentnummer und friert Vorlage, Snapshot und Renderkontext (einschließlich `business`) in derselben Transaktion ein. Ein späteres `generateDocument` verwendet diese reservierten Eingaben, auch bei Wiederholungen nach Änderungen an den Base-Einstellungen. Eine bestätigte Reservierung verbraucht ihre Nummer auch dann, wenn die Erzeugung später nicht ausgeführt wird. Die Reservierung legt auch Dateiname und Tags fest; Optionen im späteren Erzeugungsschritt ersetzen sie nicht. Andere Ziele werden ohne Reservierung geprüft. Hier wird kein PDF gerendert. Testläufe prüfen Ziele, nicht geänderte Werte.
