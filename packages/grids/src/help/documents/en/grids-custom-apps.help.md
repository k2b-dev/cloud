...
**App settings** opens a dialog with **General**, **Access**, and **Lifecycle**. Name and icon changes use the same autosaved draft; closing settings does not discard them. Saving errors remain visible in the builder.

Choose an existing entry under **Actions** to edit that sidebar action in the inspector. **New action** creates a Form action and selects it immediately. Label, icon, tone, Form, fixed values, success navigation, availability, and removal are edited there, not in App settings. The editor separates creation from existing actions; published actions keep their configured appearance.
