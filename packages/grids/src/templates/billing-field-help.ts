...
const messages = i18n.define({
  ...
  messages: {
    en: {
      ...
      ready: "Confirm only after checking the real issuer and bank details.",
      key: "Identifies the single issuer setup used by the workflows. Do not change it.",
      ...
      settings: "Your company details. Complete setup before issuing.",
      ...
      issuer_iban: "Your company account for incoming customer payments.",
    ...
    de: {
      ...
      ready: "Erst nach Prüfung der echten Unternehmens- und Bankdaten bestätigen.",
      key: "Kennzeichnet die gemeinsame Ausstellerkonfiguration der Workflows. Nicht ändern.",
      ...
      settings: "Deine Unternehmensdaten. Vor dem Ausstellen die Einrichtung abschließen.",
      ...
      issuer_iban: "Dein Unternehmenskonto für Zahlungseingänge von Kunden.",
...
export const billingFieldHelp = (locale?: string) => {
  ...
  return (table: string, key: string, type?: string): string => {
    const hintKey =
      type === "lookup"
        ? "snapshot"
        : table === "settings" && key === "iban"
          ? "issuer_iban"
          : table === "payments" && key === "reference"
            ? "payment_reference"
            : key;
