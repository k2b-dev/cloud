...
export function compileSummaryJoin(join: DslSummaryJoin, index: number, options: DslSqlCompileOptions): { ok: true; sql: unknown } | { ok: false; error: string } {
  ...
  const grouped = compileGroupQuery({
    tableId: join.tableId, fields: options.fieldsByTableId[join.tableId] ?? [],
    groupBy: query.groupBy ?? [], aggregations: [...(query.aggregations ?? []), ...(join.source.summaryFormulaAggregations ?? [])],
    filter: query.filter, extraWhere: compileRecordMetaFilter(query.recordMeta ?? null),
    includeDeleted: query.includeDeleted, deletedOnly: query.deletedOnly,
    timeZone: options.timeZone, dateConfig: options.timeZone ? { timeZone: options.timeZone } : undefined,
