...
import { cleanupFixture, ctx, insertDslDbFixture, integrationCursorSigningKey, postgresTest, preview, previewPage, uuid } from "./sql-compiler.integration-fixtures";

beforeAll(async () => { if (process.env.GRIDS_DB_TEST === "1") await migrate(); });

for (const formula of [false, true]) {
postgresTest(`independent summaries preserve equal-valued children, filters, no-child rows and pagination without fanout (formula: ${formula})`, async () => {
  ...
  try {
    ...
    const views = hydrateDslViewQueries({ ...ctx(f), views: ["Payment", "Credit"].map((status) => ({
      kind: "view" as const, id: uuid(), shortId: status, name: status, tableId: f.orders.id,
    ...
    const denied = await previewDslQuery(resolved.plan, { fieldsByTableId: context.fieldsByTableId,
      authorizedTableIds: new Set([f.customers.id]), primaryTableAuthorized: true });
  ...
  } finally { await cleanupFixture(f.baseId); }
});
}
