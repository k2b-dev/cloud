...
test("selects a mention in the middle, restores Todos, submits its identity and undoes it", async () => {
  ...
  const todo = dom.document.createElement("div"); todo.textContent = "Current task";
  const dispose = render(() => createComponent(ChatComposer, {
    get value() { return value(); }, onValueChange: setValue,
    get mentions() { return mentions(); }, onMentionsChange: setMentions,
    accessory: todo, commands: [{ name: "skill", description: "Skill", mention: { id: "one", name: "My skill", data: { ref: "one" } } }],
...
test("Escape preserves the complete draft and IME Enter never executes a command", async () => {
  ...
  let calls = 0;
  const dispose = render(() => createComponent(ChatComposer, {
    get value() { return value(); }, onValueChange: setValue, onSubmit() { calls++; },
    commands: [{ name: "compact", description: "Compact", action() { calls++; } }],
  }), dom.root);
  const input = dom.root.querySelector("textarea")!;
  input.setSelectionRange(15,15); input.click();
...
test("shows a labelled loading row and ignores results from an aborted search", async () => {
  ...
  const dispose = render(() => createComponent(ChatComposer, {
    get value() { return value(); }, onValueChange: setValue, onSubmit() {},
    searchCommands: (_query, signal) => new Promise(resolve => { pending.push({ signal, resolve }); }),
  }), dom.root);
  const input = dom.root.querySelector("textarea")!;
  input.setSelectionRange(4,4); input.click();
...
test("a command can replace the draft and request submission after its action completes", async () => {
  ...
  const dispose = render(() => createComponent(ChatComposer, {
    get value() { return value(); }, onValueChange: setValue,
    commands: [{ name: "send", description: "Send", action({ setValue, submit }) { setValue("Hello"); submit(); } }],
    onSubmit(input) { submitted.push(input.text); },
  }), dom.root);
  const input = dom.root.querySelector("textarea")!;
  input.setSelectionRange(5, 5); input.click();
