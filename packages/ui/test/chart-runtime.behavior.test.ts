...
describe("Chart runtime behavior", () => {
  ...
  test("shares exact X without redrawing SVG, preserves selection and clears only the active source", async () => {
    ...
    try {
      ...
      expect(hostB.querySelector('[role="tooltip"]')?.textContent).toContain("—");
      ...
      try {
        ...
        a.dispatchEvent(new dom.window.PointerEvent("pointerleave") as unknown as PointerEvent);
        frame?.(0);
      ...
      expect(cursor.read()?.x).toBe(1);
      expect(a.hasAttribute("aria-describedby")).toBe(true);
      b.dispatchEvent(new dom.window.KeyboardEvent("keydown", { key: "Escape", bubbles: true }) as unknown as KeyboardEvent);
      expect(cursor.read()).toBeNull();
      expect(a.hasAttribute("aria-describedby")).toBe(false);
      a.dispatchEvent(new dom.window.FocusEvent("focusin", { bubbles: true }) as unknown as FocusEvent);
      setSeries([{ label: "Traffic", data: [{ x: 1, y: 10 }] }]);
      await nextTurn();
