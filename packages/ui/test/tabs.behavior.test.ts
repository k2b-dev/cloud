...
(isServer ? test.skip : test)("pill tabs keep close and trailing controls separate and restore keyboard focus", async () => {
  ...
  const dispose = render(() => createComponent(Tabs, {
    ariaLabel: "Files", variant: "pill", value, onValueChange: setValue, trailing,
    get options() { return ids().map(id => ({ value: id, label: id, onClose: () => { setIds(ids => ids.filter(item => item !== id)); setValue("one"); } })); },
  }), dom.root);
  try {
...
(isServer ? test.skip : test)("trailing content is constructed once and keeps its interactive state", async () => {
  ...
  const [value, setValue] = createSignal("one");
  const dispose = render(() => createComponent(Tabs, {
    ariaLabel: "Files", value, onValueChange: setValue,
    options: [{ value: "one", label: "One" }, { value: "two", label: "Two" }],
    get trailing() {
      constructed++;
      const button = dom.document.createElement("button");
      button.textContent = "+";
      button.onclick = () => { button.textContent = "Open"; };
      return button;
    },
  }), dom.root);
  try {
