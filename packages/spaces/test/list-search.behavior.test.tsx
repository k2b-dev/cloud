...
describe("Spaces list search", () => {
  ...
  test("keeps focus and last-good SSR data, ignores stale responses, retries and restores history", async () => {
    ...
    try {
      ...
      const failedRetrySelection = controller.refresh();
      await flush();
      requests.at(-1)!.deferred.reject(new Error("History retry failed"));
      await failedRetrySelection;
      await flush();
      expect(new URL(dom.window.location.href).searchParams.get("item")).toBe("Latest");
      ...
      dom.window.history.replaceState(null, "", `${BASE}&q=failed-new-selection&item=Original`);
      dom.window.dispatchEvent(new dom.window.PopStateEvent("popstate"));
      await flush();
      dom.window.history.replaceState(null, "", `${BASE}&q=failed-new-selection&item=AfterBack&occurrence=Occurrence`);
      publishSpacesDetailState({ itemId: "AfterBack", occurrenceId: "Occurrence", selectionId: "AfterBack" });
      requests.at(-1)!.deferred.reject(new Error("History failed after selection"));
      await flush();
      expect(new URL(dom.window.location.href).searchParams.get("q")).toBe("history-retry");
      expect(new URL(dom.window.location.href).searchParams.get("item")).toBe("AfterBack");
      expect(new URL(dom.window.location.href).searchParams.get("occurrence")).toBe("Occurrence");
      const repeatedFailure = controller.refresh();
      await flush();
      requests.at(-1)!.deferred.reject(new Error("Retry still unavailable"));
      await repeatedFailure;
      await flush();
      expect(new URL(dom.window.location.href).searchParams.get("item")).toBe("AfterBack");
      const recoveredSelection = controller.refresh();
      await flush();
      requests.at(-1)!.deferred.resolve(result(9));
      await recoveredSelection;
      await flush();
      expect(new URL(dom.window.location.href).searchParams.get("q")).toBe("failed-new-selection");
      expect(new URL(dom.window.location.href).searchParams.get("item")).toBe("AfterBack");
