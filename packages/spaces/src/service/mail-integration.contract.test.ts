...
if (process.env.SPACES_MAIL_INTEGRATION_CHILD !== "1") {
  test("Spaces reads the current Mail mailbox and sender contracts", async () => {
    const child = Bun.spawn([process.execPath, "test", import.meta.path], {
      env: { ...process.env, SPACES_MAIL_INTEGRATION_CHILD: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
