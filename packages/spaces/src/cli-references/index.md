...
`access set` updates an existing direct grant or creates it. Revocation needs `--yes`. `--service-account` takes a service account ID or exact name; `search-principals --kind service_account` finds standalone and agent accounts by name. The `access list` table shows standalone and agent accounts, with type `agent` for agents, and hides grants of resource-bound service accounts behind Space API keys unless you pass `--include-service-accounts`; `--json` returns every entry.
