...
Press **Cmd/Ctrl+Alt+N** to create a task, or an event in calendar view, and press **Cmd/Ctrl+Shift+K** to search the current space. In Kanban, focus the board and use the arrow keys to move between cards. Press **Enter** to open the focused card, **M** to assign it to yourself, or **D** to complete it. These single-key shortcuts stay inactive while you type in a field or editor. Available actions and their shortcuts also appear in Cloud search and the layout help.
