...
// Keep the Cloud and Spaces service fixtures inside their own process so
// package-wide test runs never replace the real modules used by other suites.
if (process.env.SPACES_WORKSPACE_STATE_CHILD !== "1") {
  test("Spaces workspace SSR state authorizes, snapshots, and projects public IDs", async () => {
    const child = Bun.spawn([process.execPath, "test", import.meta.path], {
      env: { ...process.env, SPACES_WORKSPACE_STATE_CHILD: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  mock.module("@/service/resource-reference-views", () => ({
    resolveReferenceViews: async () => [],
  }));
