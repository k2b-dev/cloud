...
function file(name: string, failure = false): FileSystemEntry {
  return { name, isDirectory: false, isFile: true, file: (resolve: (file: File) => void, reject: (error: Error) => void) => {
    if (failure) reject(new Error("Permission denied")); else resolve(new File(["data"], name));
...
test("folder drops read every browser directory batch, retain relative paths and preserve empty folders", async () => {
  const result = await readDroppedEntries([directory("Docs", [[file("one.txt")], [directory("Empty", []), file("two.txt")]])], new AbortController().signal);
...
test("individual unreadable files are reported while other dropped files remain available", async () => {
  const result = await readDroppedEntries([directory("Docs", [[file("denied.txt", true), file("good.txt")]])], new AbortController().signal);
...
test("cancelling a folder scan settles even if the browser never answers its callback", async () => {
  const never = { name: "Never", isDirectory: true, isFile: false, createReader: () => ({ readEntries: () => {} }) } as unknown as FileSystemDirectoryEntry;
