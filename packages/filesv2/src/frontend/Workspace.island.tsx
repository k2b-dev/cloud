...
export default function Workspace(props: { initial: WorkspaceSnapshot; preferences?: Record<string, ViewPreference>; cloudUrl: string }) {
  ...
  const workspace = createWorkspaceState({
    ...
    load: async (source, signal) => {
      ...
      const selected = requested && url.searchParams.get("view") !== "shares"
  ...
  createEffect(() => { if (workspace.pending()) activePoll?.abort(); });
  ...
  const bases = createMemo<WorkspaceSnapshot["bases"]["items"]>((previous = []) => {
    const next = snapshot().bases.items.map((base) => {
      ...
      return known && known.name === base.name && known.status === base.status && known.kind === base.kind && known.locationKey === base.locationKey ? known : base;
    });
  ...
  const navigation = createNavigation({
    items: () => [
      ...
      { id: "shares", label: b().shares, icon: "ti ti-world-share", href: viewUrl(snapshot().selectedId, "shares"), navigation: "enhanced" as const },
    ],
    onAction: (kind) => { if (kind === "recent" || kind === "favorites") void openMarksDialog({ kind, title: kind === "recent" ? b().recent : b().favorites, onOpen: openMarked, signal: dialogLifetime.signal }); },
    onNavigate,
  });
  onMount(() =>
    onCleanup(
  ...
  const treeId = (baseId: string, path: string) => JSON.stringify([baseId, snapshot().bases.items.find(base => base.id === baseId)?.locationKey ?? null, path]);
  const pathIds = (baseId: string | null, path: string) => (baseId ? [treeId(baseId, ""), ...ancestors(path).map((item) => treeId(baseId, item))] : []);
  ...
  onCleanup(() => { for (const request of inFlight.values()) request.abort(); });
  ...
  onMount(() => {
    ...
    const timer = setInterval(async () => {
      const directory = snapshot().directory;
      if (polling || document.visibilityState !== "visible" || workspace.pending() || !directory || directory.query || currentView()) return;
      polling = true;
      const request = new AbortController();
      activePoll = request;
      ...
      try {
        ...
        for (const id of [...expanded()]) {
          ...
          if (ancestors(path).slice(0, -1).some(parent => !expanded().includes(treeId(baseId, parent)))) continue;
          const base = snapshot().bases.items.find(item => item.id === baseId && (item.locationKey ?? null) === locationKey);
        ...
        try { await go(workspace.committedSource(), true); } finally { setPreserveSelection(false); }
      } catch {
        // A failed background refresh leaves the current navigation intact; the next visible poll retries.
      } finally { clearTimeout(timeout); if (activePoll === request) activePoll = undefined; polling = false; }
    }, LIVE_REFRESH_MS);
    onCleanup(() => { activePoll?.abort(); clearInterval(timer); });
  });
  ...
  const MoreFolders = (moreProps: { baseId: string; path: string }) => {
    ...
    return <Show when={folderCursors()[id()]}>{cursor => <AppWorkspace.NavTree.Item id={`${id()}:more`} label={b().more} icon="ti ti-dots"
      onSelect={() => void loadFolders(moreProps.baseId, moreProps.path, false, cursor())} />}</Show>;
  };
    ...
    <AppWorkspace mobileSurface="flush">
      ...
      <AppWorkspace.Content>
        <Show
          ...
          fallback={<AppWorkspace.Main class="p-3"><MarksMenu kind={currentView() === "recent" ? "recent" : "favorites"} open close={() => {}} onOpen={openMarked} revision={marksRevision()} /></AppWorkspace.Main>}
        >
        <Show when={currentView() !== "shares"} fallback={<SharesOverview shares={snapshot().shares ?? { items: [], next: null }} baseId={snapshot().selectedId} onNavigate={onNavigate} />}>
          <Show
            when={!(currentView() === "trash" && selected()?.status === "existing")}
            fallback={<TrashView base={selected()!} onNavigate={onNavigate} onRestored={(path) => void go(filesUrl(selected()!.id, path.split("/").slice(0, -1).join("/"), null, path))} />}
          >
            <Show
            ...
            >
              {(directory) => (
                <Browser
                  ...
                  onBranchRefreshReady={refresh => { refreshBrowserBranches = refresh; }}
