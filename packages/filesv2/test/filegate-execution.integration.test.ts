...
// Opt-in, disposable Linux roots: never attach existing application volumes.
...
async function external(script: string) {
  await docker("run", "--rm", "--name", `${name}-writer`, "--network", "none", "-v", `${volume}:/data`, "debian:13-slim", "sh", "-eu", "-c", script);
}
...
(enabled ? describe : describe.skip)("Filegate 6.1 separate actors and indexed live Unix search", () => {
  beforeAll(async () => {
    directory = await mkdtemp(join(tmpdir(), "filesv2-execution-"));
    const token = randomUUID();
    await Bun.write(join(directory, "token"), token);
    await Bun.write(join(directory, "conf.yaml"), `server:
  listen: "0.0.0.0:4000"
...
`);
    ...
    await docker("run", "-d", "--name", name, "--user", "0:0", "--cap-drop", "ALL",
      "--cap-add", "CHOWN", "--cap-add", "DAC_OVERRIDE", "--cap-add", "FOWNER", "--cap-add", "FSETID", "--cap-add", "SETUID", "--cap-add", "SETGID",
      "--security-opt", "no-new-privileges:true", "-p", "127.0.0.1::4000", "-v", `${volume}:/data`, "-v", `${directory}:/config:ro`,
      "ghcr.io/k2b-dev/filegate:6.1.0", "serve", "--config", "/config/conf.yaml");
    ...
    for (;;) {
      try { expect((await client.system()).version).toBe("6.1.0"); break; }
      catch { if (Date.now() >= readyUntil) throw new Error(`Filegate fixture did not start: ${await docker("logs", name)}`); await Bun.sleep(250); }
    }
    cloud = client.root("cloud"); ipa = client.root("freeipa"); unix = ipa.as(identity);
  ...
  test("copies both directions without applying source credentials or ownership to the target", async () => {
    ...
    const toIpa = await cloud.transfer("cloud.txt", "freeipa", "home/from-cloud.txt", {
      targetExecution: { mode: "unix", identity }, ownership: { uid: identity.uid, gid: identity.gid, mode: "0600" },
    });
    expect(toIpa).toMatchObject({ state: "completed", node: { uid: identity.uid, gid: identity.gid } });
    const toCloud = await unix.transfer("home/ipa.txt", "cloud", "from-ipa.txt", { targetExecution: { mode: "service" }, ownership: { mode: "0600" } });
    ...
    expect((await error(() => unix.transfer("home/denied.txt", "cloud", "must-not-exist.txt", { targetExecution: { mode: "service" } })))?.status).toBe(403);
    ...
    expect((await error(() => cloud.transfer("cloud.txt", "freeipa", "locked/denied.txt", { targetExecution: { mode: "unix", identity } })))?.status).toBe(403);
  ...
  test("historical copies use independent actors and preserve current source content", async () => {
    ...
    expect((await error(() => unix.copyVersion("home/history.txt", ipaVersion.id, "cloud", "denied-history.txt", { targetExecution: { mode: "service" } })))?.status).toBe(403);
  ...
  test("indexed Unix search observes external additions without rebuilding and rejects unreadable subtrees", async () => {
    await external("mkdir /data/freeipa/home/search; echo outside > /data/freeipa/home/search/external.txt; chown -R 12345:12345 /data/freeipa/home/search; chmod 700 /data/freeipa/home/search; chmod 600 /data/freeipa/home/search/external.txt");
  ...
  test("continuations fail on deleted entries and revoked traversal instead of yielding a partial page", async () => {
    ...
    expect((await unix.search(".txt", { path: "home/pages", sort: "name" })).items.map(item => item.path)).toEqual(["home/pages/a.txt", "home/pages/c.txt"]);
    ...
    expect((await error(() => unix.search(".txt", { path: "home/pages", sort: "name", limit: 1, after: search.next })))?.status).toBe(403);
  }, 60_000);
});
