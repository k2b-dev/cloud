...
export function EditorWorkspace(props: {
  ...
  const locale = useLocale(), t = () => messages.resolve([locale()]).t;
  const [ratio, setRatio] = createSignal(.55);
  const [mobile, setMobile] = createSignal("code");
  const owner = getOwner();
  const ids = new Map<string, string>();
  const fileId = (path: string) => { if (!ids.has(path))
    ids.set(path, `file-${ids.size}`); return ids.get(path)!; };
  ...
  onCleanup(() => { for (const session of sessions.values())
    session.dispose(); });
  const view = createMemo(() => {
    ...
    const layout: PanesLayout = { version: 2, root: left && right ? { type: "split", direction: "horizontal", ratio: ratio(), first: left, second: right } : left ?? right };
    return { items, layout };
  });
  return <div class="kit-editor-workspace" data-mobile={mobile()}>
  <div class="kit-editor-mobile-tabs" aria-label={t().workspace}>
   <Button variant="ghost" aria-pressed={mobile() === "code"} onClick={() => setMobile("code")}>{t().code}</Button>
   <Button variant="ghost" aria-pressed={mobile() === "preview"} onClick={() => setMobile("preview")}>{t().preview}</Button>
  </div>
  <Panes class="kit-editor-panes" ariaLabel={t().workspace} layout={view().layout} items={view().items} split={false} movable={false} onAddItem={id => props.onAdd(id?.startsWith("preview-") ?? false)} onLayoutChange={async (next) => {
      const root = next.root;
      if (root?.type !== "split" || root.first.type !== "group" || root.second.type !== "group")
        return;
      const rightActive = root.second.active, leftActive = root.first.active;
      const selected = props.previews.find(entry => `preview-${fileId(entry.path)}` === rightActive)?.path;
      if (!selected)
        return;
      if (selected !== props.selectedPreview && !(await props.onPreviewSelect(selected)))
        return;
      setRatio(root.ratio);
