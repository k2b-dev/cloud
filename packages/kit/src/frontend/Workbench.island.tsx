...
export default function Workbench(props: { project: Bundle; userId: string; edit: boolean; entry: string; access: AccessEntry[] }) {
  ...
  const validation = createMemo(() => {
    ...
    try {
      ...
      return displayError(error, locale());
    }
  });
  ...
  const save = mutation.create({
    ...
    onError: (e) => {
      setConflict(e instanceof KitRequestError && e.code === "REVISION_CONFLICT");
  ...
  async function start() {
    ...
    try {
      ...
      run = startRun(container, code, new AppStorage(props.userId, saved().id, persistent()), {
        modal: (request, signal) => openKitModal(request, signal, locale()),
        database: createDatabaseHost(saved().id, locale()),
        ui: setNodes,
        ...
        save: async (data, filename) => {
          files.downloadFileFromContent(
            data,
            filename,
            typeof data === "string" ? "text/csv;charset=utf-8" : data.type || "application/octet-stream",
          );
        },
      });
      setActive(true);
  ...
  async function addFile(entryOnly = false, markdown = false) {
    ...
    batch(() => {
      setSource((files) => [
        ...
        {
          path,
          content: path.endsWith(".script.js")
            ? 'export default kit.script({ name: "New tool", run() { kit.ui.text("Hello"); } });\n'
            : isPage(path)
              ? "# Documentation\n\n"
              : "",
        },
  ...
  async function removeFile(path: string) {
    if (isNavigationFile(path) && entries().length === 1) {
      setError(t().lastEntry);
      return;
    }
    if (!(await prompts.confirm(`${t().deleteFile}: ${path}?`, { title: t().deleteFile, variant: "danger", confirmText: t().deleteFile })))
      return;
  ...
  async function renameFile(from: string) {
    ...
    try {
      ...
      setError(`${t().renameBlocked} ${displayError(error, locale())}`);
    }
  }
  function downloadDraft() {
    files.downloadFileFromContent(
      JSON.stringify({ ...input(), expectedRevision: saved().revision }, null, 2),
      `${saved().id}-draft.json`,
      "application/json",
    );
  }
    ...
    return (
      <>
        ...
        <Show when={props.edit}>
          ...
          <AppWorkspace.SidebarItem icon="ti ti-file-plus" tone="success" onClick={() => addFile(false, true)}>
            {t().addPage}
          </AppWorkspace.SidebarItem>
        </Show>
      </>
  ...
  async function localData() {
    ...
    try {
      await prompts.dialog<void>(
        ...
        { title: t().localData, icon: "ti ti-folder", size: "large" },
      );
    } finally {
      setSettingsOpen(false);
    }
                  ...
                  <For each={logs()}>
                    {(entry) => (
                      <span class="kit-console-line" data-level={entry.level}>
                        <span class="kit-console-time">{entry.time}</span> <span class="kit-console-level">{logLabel(entry.level)}:</span>{" "}
                        {entry.text}
                        {"\n"}
                      </span>
                    )}
    ...
    <AppWorkspace class={`kit-workspace ${props.edit ? "kit-edit" : "kit-use"}`}>
      ...
      <AppWorkspace.Content>
        <AppWorkspace.Main class="kit-main" scroll={!props.edit}>
          <div class="kit-toolbar">
            ...
            <div class="kit-flow kit-flow-row kit-gap-sm">
              ...
              <Show when={props.edit}>
                ...
                <Button
                  onClick={() => {
                    if (!save.loading()) save.mutate(undefined);
                  }}
          ...
          <Show when={props.edit && validation()}>
            <div class="kit-validation" role="status" aria-label={t().validationTitle}>
              {validation()}
            </div>
          </Show>
