...
// Keep the capability transport fixture inside its own process so package-wide
// test runs never replace the real capability server used by other suites.
if (process.env.MAIL_CALENDAR_DESTINATIONS_CHILD !== "1") {
  test("Mail reads the current Spaces calendar destination contract", async () => {
    const child = Bun.spawn([process.execPath, "test", import.meta.path], {
      env: { ...process.env, MAIL_CALENDAR_DESTINATIONS_CHILD: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
