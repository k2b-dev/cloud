...
import { resolveIdentityJwksUrl, resolveInvocationJwksUrl, resolveOAuthJwksUrl, resolveSessionJwksUrl } from "./runtime-config";

describe("identity JWKS transport", () => {
  test("uses the public issuer when no private transport origin is configured", () => {
    expect(resolveIdentityJwksUrl("https://cloud.example", undefined).href).toBe(`https://cloud.example${CLOUD_IDENTITY_JWKS_PATH}`);
  ...
  test("keeps the public issuer independent from a private HTTP transport", () => {
    expect(resolveIdentityJwksUrl("https://cloud.example", "http://app-core:3000").href).toBe(
      `http://app-core:3000${CLOUD_IDENTITY_JWKS_PATH}`,
    );
  ...
  test("rejects non-HTTP transports", () => {
    expect(() => resolveIdentityJwksUrl("https://cloud.example", "file:///tmp/core")).toThrow(
