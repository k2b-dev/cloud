...
export default function GlobalSearchDialog(props: GlobalSearchDialogProps) {
  ...
  return <CloudResourceSearch navigationItems={props.searchLinks} helpApps={props.helpApps} onHelp={openHelp} onSelect={openItem} />;
}

export const openGlobalSearchDialog = (helpApps: GlobalSearchHelpApp[] = [], searchLinks: NavigationSearchItem[] = []) => {
  if (dialogCore.isOpen()) return;

  void dialogCore.open<void>((close) => <GlobalSearchDialog searchLinks={searchLinks} close={close} helpApps={helpApps} />, {
    panelClassName:
