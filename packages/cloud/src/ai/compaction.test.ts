...
describe("compaction inference accounting", () => {
  test("Nessi compacts an oversized first tool result before requesting its second model turn", async () => {
    ...
    let requests = 0;
    const provider: Provider = {
      ...fixture.context.provider, contextWindow:1000,
    ...
    const loop = nessi({provider,systemPrompt:"Analyze",input:"Read the file",maxTurns:2,
      tools:[defineTool({name:"read",description:"Read",inputSchema:z.object({})}).server(async()=>"LARGE_FILE_PAYLOAD".repeat(500))],
      compact:fixture.compact,store:{load:async()=>history,append:async message=>{history.push({seq:++seq,kind:"message",message});}},
    });
    for await (const _event of loop) { /* Drain the real Nessi event loop. */ }
  ...
  test("compacts completed tool rounds during the first user loop", async () => {
    ...
    const firstLoop: StoreEntry[] = [{ seq: 1, kind: "message", message: { role: "user", content: [{ type: "text", text: "Build the dashboard" }] } }];
    for (let index = 0; index < 4; index++) {
      firstLoop.push({ seq: firstLoop.length + 1, kind: "message", message: { role: "assistant", content: [{ type: "tool_call", id: `call-${index}`, name: "read_file", args: {} }] } });
      firstLoop.push({ seq: firstLoop.length + 1, kind: "message", message: { role: "tool_result", callId: `call-${index}`, name: "read_file", result: "data" } });
    }
  ...
  test("can summarize one oversized completed tool round before the second model request", async () => {
    ...
    const firstLoop: StoreEntry[] = [
      ...
      {seq:2,kind:"message",message:{role:"assistant",content:[{type:"tool_call",id:"read",name:"read_file",args:{}}]}},
  ...
  test("never archives an unanswered tool call across the split", async () => {
    ...
    const firstLoop: StoreEntry[] = [
      { seq: 1, kind: "message", message: { role: "user", content: [{ type: "text", text: "Build" }] } },
      { seq: 2, kind: "message", message: { role: "assistant", content: [{ type: "tool_call", id: "pending", name: "code_run", args: {} }] } },
      ...[3, 4, 5].map(seq => ({ seq, kind: "message" as const, message: { role: "assistant" as const, content: [{ type: "text" as const, text: "Working" }] } })),
    ];
