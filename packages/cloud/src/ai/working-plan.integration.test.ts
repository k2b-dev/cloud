...
databaseSuite()("working plan and execution budget in disposable Postgres", () => {
  ...
  beforeAll(async () => {
    fresh = await useFreshDatabase("working_plan");
    await migrateAuth();
  ...
  test("plan survives compaction and fork, rolls back on retry, ignores failed updates", async () => {
    ...
    await aiConversations.compactMessages({
      ...
      summary: { role: "assistant", content: [{ type: "text", text: "A short handoff" }] },
    });
  ...
  test("zero budget has no deadline, lease reclaim keeps the original budget", async () => {
    for (const budget of [0, 30 * 60_000]) {
      ...
      const { turn } = await aiConversations.submitChatTurn({
        ...
        runConfig: { kind: "chat", input: "work", toolSource: { kind: "none" } },
        userMessage: { role: "user", content: [{ type: "text", text: "work" }] },
      });
      ...
      const resumed = await aiConversations.claimTurn({
        conversationId: chat.id,
        turnId: turn.id,
        from: "queue",
        leaseOwner: "fixture-2",
        leaseMs: 45000,
        maxAttempts: 5,
        runBudgetMs: 60000,
      });
