...
export async function withAiSkillSearch<T>(query: string, run: (bm25: boolean) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    const capabilityLost = typeof error === "object" && error !== null &&
      ["code", "errno"].some(key => key in error &&
        ["0A000", "42704", "42883", "55000"].includes(String(Reflect.get(error, key))));
...
export function aiSkillSearchSql(query: string, bm25: boolean) {
  ...
  const matches = !query ? sql`TRUE` : !terms.length ? sql`FALSE` : sql`
  ...
  `;
  const rank = !query ? sql`0::real` : sql`
  ...
  `;
  const textRank = !query ? sql`0::real` : bm25
