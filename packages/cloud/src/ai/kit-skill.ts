...
For shared data, read the Kit database Help first. The user or app administrator explicitly enables the database; source changes must not enable it. Use database.status for its generation, database.read for inspection and database.write for approved schema/row changes. Schema changes require Admin. Prefer sequential imports; never replay ambiguous writes. Most Kit apps have one active user: do not add locks, queues or conflict machinery without an actual multi-user requirement. For shared edits, use constraints and do not mistake read-then-write for an atomic operation.
