...
import { buildBlocksFromMessages, type AiTurnBlock } from "../protocol";

type Tool = Extract<AiTurnBlock, { kind: "tool" }>;
const tableResult = (name: string) => ({ data: { rows: [{ name }] }, presentation: { kind: "table", rowsPath: ["rows"], columns: [{ path: ["name"], label: "Name" }] } });
const capability: Tool = { id: "tool-query", callId: "query", kind: "tool", name: "grids__query__gql_dot_execute", status: "running", args: {}, presentation: { kind: "capability", appId: "grids", appName: "Grids", appIcon: "ti ti-table", title: "Query result", capabilityKind: "query" } };
...
domTest("capability tables become visible after approval and update outside tool summaries", async () => {
  ...
  const [blocks, setBlocks] = createSignal<AiTurnBlock[]>([{ ...capability, status: "awaiting_approval", approval: { message: "Allow query", allowAlways: false } }]);
...
domTest("persisted table results render without optional capability branding", async () => {
  ...
  const blocks = buildBlocksFromMessages([
    ...
    { seq: 2, message: { role: "tool_result", callId: "query", name: capability.name, result: tableResult("Saved result"), isError: false } },
  ]);
  const dispose = render(() => <AiTurnBlockList blocks={blocks} turnId="turn" />, dom.root);
  try {
    expect(dom.root.querySelectorAll("table").length).toBe(1);
