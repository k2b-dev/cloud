...
(isServer ? test.skip : test)("streamed text and tool updates render each block exactly once", async () => {
  ...
  const dispose = render(() => <AiChatActionsProvider actions={{}}>{(() => {
  ...
  })()}</AiChatActionsProvider>, dom.root);
  ...
  try {
    ...
    for (let index = 0; index < 10; index++) {
      emit({ ...base, seq: 2 + index * 3, type: "block_delta", blockId: `text-${index}`, blockKind: "text", delta: `Unique message ${index}.` });
      emit({ ...base, seq: 3 + index * 3, type: "block_set", block: { id: `tool-${index}`, kind: "tool", callId: `call-${index}`, name: "code_run", args: {}, status: "running" } });
      await new Promise(resolve => setTimeout(resolve, 0));
      for (let previous = 0; previous <= index; previous++) expect(dom.root.textContent?.split(`Unique message ${previous}.`).length).toBe(2);
      emit({ ...base, seq: 4 + index * 3, type: "block_set", block: { id: `tool-${index}`, kind: "tool", callId: `call-${index}`, name: "code_run", args: {}, status: "completed", result: {} } });
    }
    const old = structuredClone(unwrap(state.activeTurn))!;
    const baseline = { ...old, seq: 100, blocks: old.blocks.map((block) => ({ ...block, id: block.kind === "text" ? `persisted-${block.id}` : block.id })) };
