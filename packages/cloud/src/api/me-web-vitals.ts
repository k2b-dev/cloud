...
export const createMeWebVitalsRoutes = (
  record: (report: WebVitalsReport) => void = (report) => log.info(report.name, report),
  isEnabled: () => Promise<boolean> = async () => (await get("observability.web_vitals.enabled")) === true,
) =>
  new Hono<AuthContext>().post(
    ...
    describeRoute({
      ...
      ...requiresAuth,
      responses: {
        204: { description: "Report processed; discarded when collection is disabled" },
    ...
    async (c) => {
      ...
      if (await isEnabled()) record(c.req.valid("json"));
