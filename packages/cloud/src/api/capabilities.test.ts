...
test("ordinary invocation seals a stream offer and the transfer route accepts only that caller", async () => {
  ...
  const streamDefinition = compileCapabilities(
    "demo",
    defineCapabilities({
      protocolVersion: 2,
      queries: {
        content: {
          ...
          data: z.object({}),
          openWorld: false,
          stream: { direction: "read", maxBytes: 3, read: async () => new Response("abc") },
          run: () => ok({ data: {} }),
        },
      },
    }),
  ...
  const result = await dispatchCapability({
    request: new Request("http://cloud.test/api/capabilities/v1/queries/demo/content"),
    kind: "queries",
    appId: "demo",
  ...
  const response = await dispatchCapabilityStream(
    new Request("http://cloud.test/api/capabilities/v1/streams/read", { headers: { "x-cloud-stream-id": body.stream.id } }),
    resourceAuthority,
    "read",
    {
