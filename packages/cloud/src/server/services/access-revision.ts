...
import type { Principal, PermissionLevel } from "./access";
...
export function accessRevision(entries: ReadonlyArray<{ id: string; principal: Principal; permission: PermissionLevel }>): string {
  const state = entries.map(entry => [entry.id, entry.permission,
    Object.entries(entry.principal).sort(([left], [right]) => left.localeCompare(right))])
