...
import type { CapabilityCatalogApp, CapabilityCatalogAppClientResult, CapabilityCatalogClientResult } from "@k2b/cloud/capabilities/server";
...
const app = (appId: string, appName = appId): CapabilityCatalogApp => ({
  ...
  manifest: { protocolVersion: 2, appId, manifestHash: hash, types: [], queries: [], actions: [], commands: [] },
});
...
describe("capability workspace catalog", () => {
  test("loads every catalog page for the workspace app list", async () => {
    ...
    const pages = new Map<string | undefined, CapabilityCatalogClientResult>([
      [undefined, { ok: true, data: { protocolVersion: 2, apps: [app("zeta", "Zeta")], page: { hasMore: true, nextCursor: "page-2" } } }],
      ["page-2", { ok: true, data: { protocolVersion: 2, apps: [app("alpha", "Alpha")], page: { hasMore: false } } }],
    ]);
