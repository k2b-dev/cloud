...
`--include-service-accounts`. JSON output always contains every entry. Return
entries from `resolveDisplayNames()`, or set `serviceAccountKind` yourself, so
the table can tell these accounts apart.
